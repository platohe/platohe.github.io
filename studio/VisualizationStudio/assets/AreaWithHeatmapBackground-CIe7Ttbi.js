...
export const meta = {\r
  id: 'area-with-heatmap-background',\r
  title: 'Area With Heatmap Background',\r
  desc: 'Area With Heatmap Background — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithHeatmapBackground',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis","d3-color"],\r
  tags: ["areas","area-with-heatmap-background"],\r
}\r
\r
export default function AreaWithHeatmapBackground({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Heatmap background\r
    const heatmapData=Array.from({length:20},(_,i)=>({y:i*4, intensity:Math.random()}))\r
    g.selectAll('rect.heat').data(heatmapData).enter().append('rect')\r
      .attr('class','heat').attr('x',0).attr('width',width).attr('height',height/20)\r
      .attr('y',d=>y(d.y+2)).attr('fill',d=>d3.interpolateYlOrRd(d.intensity)).attr('opacity',0.15)\r
    ...
    g.append('path').datum(curveData).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis))\r
      .attr('fill',colors[0]).attr('fill-opacity',0.22).attr('stroke',colors[0]).attr('stroke-width',1.4)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Heatmap Background')\r
