...
export const meta = {\r
  id: 'area-with-interior-point-path',\r
  title: 'Area With Interior Point Path',\r
  desc: 'Area With Interior Point Path — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithInteriorPointPath',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-interior-point-path"],\r
}\r
\r
export default function AreaWithInteriorPointMethod({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    curveData.forEach((d,i)=>{\r
      ...
      const next=d.y + direction*2\r
      path.push({x:d.x, y:Math.max(0,Math.min(80,next))})\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Interior Point Path')\r
