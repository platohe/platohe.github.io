...
// DendrogramColored: Color-coded clades.\r
export const meta = {\r
  id: 'dendrogram-colored',\r
  title: 'Dendrogram Colored',\r
  desc: 'Dendrogram Colored — a hierarchies chart visualization',\r
  category: 'Hierarchies',\r
  component: 'DendrogramColored',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["hierarchies","dendrogram-colored"],\r
}\r
\r
export default function DendrogramColored({ data: customData }) {\r
  ...
  useEffect(() => {\r
    ...
    g.selectAll('path.link').data(root.links()).join('path')\r
      .attr('fill', 'none')\r
      .attr('stroke', (d) => { let top = d; while (top.depth > 1) top = top.parent; return colors[root.children.indexOf(top) % colors.length] })\r
    ...
    nodes.append('circle').attr('r', (d) => d.children ? 4 : 3).attr('fill', (d) => { let top = d; while (top.depth > 1 && root.children.indexOf(top) < 0) top = top.parent; return colors[Math.max(root.children.indexOf(top), 0) % colors.length] }).attr('stroke', 'var(--bg)')\r
