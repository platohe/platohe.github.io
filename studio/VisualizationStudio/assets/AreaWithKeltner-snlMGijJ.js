...
export const meta = {\r
  id: 'area-with-keltner',\r
  title: 'Area With Keltner',\r
  desc: 'Area With Keltner — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithKeltner',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-keltner"],\r
}\r
\r
export default function AreaWithKeltner({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Keltner channels\r
    const window=20\r
    const channels=curveData.map((d,i)=>{\r
      const slice=curveData.slice(Math.max(0,i-19),i+1)\r
      const vals=slice.map(d=>d.y)\r
      const mean=d3.mean(vals)||0\r
      const atr=d3.mean(slice.map(d=>Math.abs(d.y-(d3.mean(slice.map(d=>d.y))||0))))||0\r
      return {x:d.x, mean, upper:mean+1.5*atr, lower:mean-1.5*atr}\r
    })\r
    const area=d3.area().x(d=>x(d.x)).y0(d=>y(d.upper)).y1(d=>y(d.lower)).curve(d3.curveBasis)\r
    g.append('path').datum(channels).attr('d',area).attr('fill',colors[0]).attr('fill-opacity',0.12).attr('stroke',colors[0]).attr('stroke-width',1)\r
    ...
    g.append('path').datum(channels).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.mean)).curve(d3.curveBasis)).attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1).attr('stroke-dasharray','3,2')\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Keltner Channels')\r
