...
export const meta = {\r
  id: 'area-with-actor-critic',\r
  title: 'Area With Actor Critic',\r
  desc: 'Area With Actor Critic — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithActorCritic',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-actor-critic"],\r
}\r
\r
export default function AreaWithActorCritic({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Actor-Critic\r
    const actor=curveData.map((d,i)=>({x:d.x, y:d.y+(Math.random()-0.5)*4}))\r
    const critic=curveData.map((d,i)=>({x:d.x, y:42+(Math.random()-0.5)*2}))\r
    ...
    g.append('path').datum(critic).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1.2).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Actor-Critic')\r
