...
// HealthStatsDashboard: KPI tiles + mini charts composition.\r
export const meta = {\r
  id: 'health-stats-dashboard',\r
  title: 'Health Stats Dashboard',\r
  desc: 'Health Stats Dashboard — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'HealthStatsDashboard',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","health-stats-dashboard"],\r
}\r
\r
export default function HealthStatsDashboard({ data: customData }) {\r
  ...
  useEffect(() => {\r
    ...
    g.append('text').attr('x', 24).attr('y', 34).attr('font-size', '11px').attr('font-weight', 700)\r
      .attr('fill', 'var(--text-secondary)').text('HealthStats · This Week')\r
    const kpis = [["Patients","1,204",4],["Wait","18m",-14],["Bed Use","82%",3]]\r
    kpis.forEach((k, i) => {\r
      const x = 24 + i * 122\r
    ...
    const bars = [["ER",36],["Clinic",44],["Tele",20]]\r
    let by = 178\r
    bars.forEach((b, i) => {\r
