...
export const meta = {\r
  id: 'area-with-meta-learning',\r
  title: 'Area With Meta Learning',\r
  desc: 'Area With Meta Learning — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithMetaLearning',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-meta-learning"],\r
}\r
\r
export default function AreaWithMetaLearning({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Meta-learning: learning to forecast\r
    const tasks=5\r
    const tasksData=Array.from({length:5},()=>curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*4})))\r
    const metaLearned=curveData.map((_,i)=>({x:curveData[i].x, y:d3.mean(tasksData.map(t=>t[i].y))}))\r
    ...
    g.append('path').datum(curveData.map((_,i)=>({x:curveData[i].x, y:d3.mean(tasksData.map(t=>t[i].y))}))).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#22c55e').attr('stroke-width',1.4).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Meta-Learning')\r
