...
export const meta = {\r
  id: 'density-with-rug',\r
  title: 'Density With Rug',\r
  desc: 'Density With Rug — a analysis chart visualization',\r
  category: 'Analysis',\r
  component: 'DensityWithRug',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["analysis","density-with-rug"],\r
}\r
\r
export default function DensityWithRug({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    allKde.forEach((kde,i)=>{\r
      ...
      g.append('path').datum(kde).attr('d',area).attr('fill',colors[i]).attr('fill-opacity',0.14)\r
      g.append('path').datum(kde).attr('d',line).attr('fill','none').attr('stroke',colors[i]).attr('stroke-width',1.6)\r
      // rug\r
      raw[i].forEach(v=> g.append('line').attr('x1',x(v)).attr('x2',x(v)).attr('y1',height-14).attr('y2',height-10).attr('stroke',colors[i]).attr('stroke-width',0.8).attr('opacity',0.62))\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Density with Rug')\r
  },[customData])\r
