...
export const meta = {\r
  id: 'area-with-epsilon-greedy',\r
  title: 'Area With Epsilon Greedy',\r
  desc: 'Area With Epsilon Greedy — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithEpsilonGreedy',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-epsilon-greedy"],\r
}\r
\r
export default function AreaWithEpsilonGreedy({ data: customData, onCurveData }) {\r
  ...
  useEffect(()=>{\r
    ...
    curveData.forEach((d,i)=>{\r
      ...
      g.append('rect').attr('x',x(d.x)).attr('y',y(d.y)).attr('width',width/3).attr('height',height-y(d.y))\r
        .attr('fill',colors[arm%colors.length]).attr('fill-opacity',0.6).attr('stroke',colors[arm]).attr('stroke-width',1)\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Epsilon-Greedy')\r
