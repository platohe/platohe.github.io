...
export const meta = {\r
  id: 'violin-with-deviation',\r
  title: 'Violin With Deviation',\r
  desc: 'Violin With Deviation — a analysis chart visualization',\r
  category: 'Analysis',\r
  component: 'ViolinWithDeviation',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["analysis","violin-with-deviation"],\r
}\r
\r
export default function ViolinWithDeviation({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    groups.forEach((gr,i)=>{\r
      ...
      g.append('line').attr('x1',cx-8).attr('x2',cx+8).attr('y1',y(avg)).attr('y2',y(avg)).attr('stroke',dev>=0?'#22c55e':'#ef4444').attr('stroke-width',1.2).attr('stroke-dasharray','2,2')\r
      g.append('text').attr('x',cx).attr('y',y(avg)-6).attr('text-anchor','middle').attr('fill',dev>=0?'#22c55e':'#ef4444').attr('font-size','6px').text(\`\${dev>=0?'+':''}\${dev.toFixed(1)}\`)\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Violin with Deviation')\r
  },[customData])\r
