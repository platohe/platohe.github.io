...
export const meta = {\r
  id: 'area-with-markov-switching',\r
  title: 'Area With Markov Switching',\r
  desc: 'Area With Markov Switching — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithMarkovSwitching',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-markov-switching"],\r
}\r
\r
export default function AreaWithMarkovSwitching({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Markov regime switching\r
    const regimes=[{mean:35,color:'#ef4444'},{mean:48,color:'#22c55e'},{mean:42,color:'#3b82f6'}]\r
    let currentRegime=0\r
    const regimeData=curveData.map((d,i)=>{\r
      if(i>0 && Math.random()<0.02) currentRegime=(currentRegime+1)%3\r
      return {x:d.x, y:d.y, regime:currentRegime}\r
    })\r
    regimes.forEach(r=>{\r
      const regimeRows=regimeData.filter(d=>d.regime===regimes.indexOf(r))\r
      g.append('path').datum(regimeRows).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis))\r
        .attr('fill',r.color).attr('fill-opacity',0.15).attr('stroke',r.color).attr('stroke-width',1.2)\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Markov Switching')\r
