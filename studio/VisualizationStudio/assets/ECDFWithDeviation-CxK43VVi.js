...
export const meta = {\r
  id: 'ecdfwith-deviation',\r
  title: 'E C D F With Deviation',\r
  desc: 'E C D F With Deviation — a analysis chart visualization',\r
  category: 'Analysis',\r
  component: 'ECDFWithDeviation',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["analysis","e-c-d-f-with-deviation"],\r
}\r
\r
export default function ECDFWithDeviation({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    g.append('line').attr('x1',x(avg)).attr('x2',x(avg)).attr('y1',0).attr('y2',height).attr('stroke','#f59e0b').attr('stroke-width',1).attr('stroke-dasharray','3,2').attr('opacity',0.72)\r
    const avgY=pts.find(p=>p.x>=avg)?.y||0.5\r
    g.append('circle').attr('cx',x(avg)).attr('cy',y(avgY)).attr('r',3).attr('fill','#f59e0b').attr('stroke','var(--bg)')\r
    g.append('text').attr('x',x(avg)+4).attr('y',y(avgY)-6).attr('fill','#f59e0b').attr('font-size','7px').text(\`avg \${avg.toFixed(1)}\`)\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('ECDF with Deviation')\r
  },[customData])\r
