...
export const meta = {\r
  id: 'area-with-dash-pattern',\r
  title: 'Area With Dash Pattern',\r
  desc: 'Area With Dash Pattern — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithDashPattern',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-dash-pattern"],\r
}\r
\r
export default function AreaWithDashPattern({ data: customData, onCurveData })\r
{\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Dashed pattern fill\r
    const defs=svg.append('defs')\r
    const pattern=defs.append('pattern').attr('id','dashPattern').attr('patternUnits','userSpaceOnUse').attr('width',10).attr('height',10)\r
    pattern.append('rect').attr('width',10).attr('height',10).attr('fill',colors[0]).attr('fill-opacity',0.15)\r
    pattern.append('line').attr('x1',0).attr('y1',10).attr('x2',10).attr('y2',0).attr('stroke','#fff').attr('stroke-width',1.5).attr('stroke-dasharray','3,2')\r
    const area=d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)\r
    g.append('path').datum(curveData).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)).attr('fill','url(#dashPattern)').attr('stroke',colors[0]).attr('stroke-width',1.6)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Dash Pattern')\r
