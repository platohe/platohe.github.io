...
export const meta = {\r
  id: 'area-with-double-exponential-smoothing',\r
  title: 'Area With Double Exponential Smoothing',\r
  desc: 'Area With Double Exponential Smoothing — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithDoubleExponentialSmoothing',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-double-exponential-smoothing"],\r
}\r
\r
export default function AreaWithDoubleExponentialSmoothing({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Double exponential smoothing (Holt's method)\r
    const alpha=0.3, beta=0.1\r
    let level=curveData[0].y, trend=0\r
    const smoothed=[]\r
    curveData.forEach(d=>{\r
      level=0.3*d.y+0.7*(level+trend)\r
      trend=0.1*(level-smoothed[smoothed.length-1]||0)+0.9*trend\r
      smoothed.push({x:d.x, y:level+trend})\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Double Exponential Smoothing')\r
