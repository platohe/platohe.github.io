...
// OperationsMonitorDashboard: KPI tiles + mini charts composition.\r
export const meta = {\r
  id: 'operations-monitor-dashboard',\r
  title: 'Operations Monitor Dashboard',\r
  desc: 'Operations Monitor Dashboard — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'OperationsMonitorDashboard',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","operations-monitor-dashboard"],\r
}\r
\r
export default function OperationsMonitorDashboard({ data: customData }) {\r
  ...
  useEffect(() => {\r
    ...
    g.append('text').attr('x', 24).attr('y', 34).attr('font-size', '11px').attr('font-weight', 700)\r
      .attr('fill', 'var(--text-secondary)').text('OperationsMonitor · This Week')\r
    const kpis = [["Uptime","99.97%",0],["Queue","214",-18],["Errors","0.4%",-12]]\r
    kpis.forEach((k, i) => {\r
      const x = 24 + i * 122\r
    ...
    const bars = [["API",46],["DB",27],["Cache",27]]\r
    let by = 178\r
    bars.forEach((b, i) => {\r
