...
export const meta = {\r
  id: 'area-with-ridge-regression',\r
  title: 'Area With Ridge Regression',\r
  desc: 'Area With Ridge Regression — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithRidgeRegression',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-ridge-regression"],\r
}\r
\r
export default function AreaWithRidgeRegression({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Ridge regression (L2 regularization)\r
    const lambda=0.1\r
    const trend=[]\r
    let prev=0\r
    curveData.forEach(d=>{\r
      const raw=d.y\r
      const smoothed=prev + (raw-prev)*0.3\r
      trend.push({x:d.x, y:smoothed})\r
      prev=smoothed\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Ridge Regression')\r
