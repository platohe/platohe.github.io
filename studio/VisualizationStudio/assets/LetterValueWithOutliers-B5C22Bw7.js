...
export const meta = {\r
  id: 'letter-value-with-outliers',\r
  title: 'Letter Value With Outliers',\r
  desc: 'Letter Value With Outliers — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'LetterValueWithOutliers',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","letter-value-with-outliers"],\r
}\r
\r
export default function LetterValueWithOutliers({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    raw.forEach((vals,i)=>{\r
      ...
      vals.filter(v=>v<lo||v>hi).forEach(v=> g.append('circle').attr('cx',cx+(Math.random()-0.5)*12).attr('cy',y(v)).attr('r',1.8).attr('fill','#ef4444').attr('stroke','var(--bg)').attr('stroke-width',0.6))\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Letter-Value with Outliers')\r
  },[customData])\r
