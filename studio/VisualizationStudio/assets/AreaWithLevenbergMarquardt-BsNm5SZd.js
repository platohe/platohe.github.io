...
export const meta = {\r
  id: 'area-with-levenberg-marquardt',\r
  title: 'Area With Levenberg Marquardt',\r
  desc: 'Area With Levenberg Marquardt — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithLevenbergMarquardt',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-levenberg-marquardt"],\r
}\r
\r
export default function AreaWithLevenbergMarquardt({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Levenberg-Marquardt curve fitting\r
    const trend=curveData.map((_,i)=>({x:i, y: curveData[i].y + (Math.random()-0.5)*2}))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Levenberg-Marquardt')\r
