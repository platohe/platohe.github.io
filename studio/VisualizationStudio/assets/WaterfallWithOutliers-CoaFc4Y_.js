...
export const meta = {\r
  id: 'waterfall-with-outliers',\r
  title: 'Waterfall With Outliers',\r
  desc: 'Waterfall With Outliers — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'WaterfallWithOutliers',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","waterfall-with-outliers"],\r
}\r
\r
export default function WaterfallWithOutliers({ data: customData }) {\r
...
\r
    data.forEach(d=>{\r
      const isOut=!d.isTotal && (d.v<lo||d.v>hi)\r
      const col=d.isTotal?'#0f172a': (isOut?'#ef4444': (d.y1>=d.y0?'#22c55e':'#ef4444'))\r
      g.append('rect').attr('x',safeX(d.label)??0).attr('y',safeY(Math.max(d.y0,d.y1))).attr('width',x.bandwidth()).attr('height',Math.max(0, Math.abs(safeY(d.y0)-safeY(d.y1)))).attr('fill',col).attr('fill-opacity',isOut?0.92:0.62).attr('rx',3).attr('stroke','var(--bg)')\r
      if(isOut) g.append('text').attr('x',(safeX(d.label)??0)+x.bandwidth()/2).attr('y',safeY(Math.max(d.y0,d.y1))-4).attr('text-anchor','middle').attr('fill','#ef4444').attr('font-size','6px').text('outlier')\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Waterfall with Outliers')\r
  },[customData])\r
