...
export const meta = {\r
  id: 'ridge-with-ci',\r
  title: 'Ridge With C I',\r
  desc: 'Ridge With C I — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'RidgeWithCI',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","ridge-with-c-i"],\r
}\r
\r
export default function RidgeWithCI({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    groups.forEach((gr,i)=>{\r
      ...
      const ciArea=d3.area().x(d=>x(d.v)).y0(yy).y1(d=>yy - h(d.d)*0.68).curve(d3.curveBasis)\r
      g.append('path').datum(kde).attr('d',area).attr('fill',colors[i]).attr('fill-opacity',0.08)\r
      g.append('path').datum(kde).attr('d',ciArea).attr('fill',colors[i]).attr('fill-opacity',0.22).attr('stroke',colors[i]).attr('stroke-width',1)\r
      const m=d3.median(vals)||50\r
      g.append('circle').attr('cx',x(m)).attr('cy',yy - h(kde.find(k=>Math.abs(k.v-m)<1)?.d||0)/2).attr('r',2).attr('fill',colors[i]).attr('stroke','var(--bg)')\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Ridge with CI')\r
  },[customData])\r
