...
export const meta = {\r
  id: 'area-with-faceted-axes',\r
  title: 'Area With Faceted Axes',\r
  desc: 'Area With Faceted Axes — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithFacetedAxes',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-faceted-axes"],\r
}\r
\r
export default function AreaWithFacetedAxes({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Faceted axes: secondary y-axis on right\r
    const y2=d3.scaleLinear().domain([0,100]).range([height,0])\r
    ...
    g.append('g').attr('transform',\`translate(\${width},0)\`).call(d3.axisRight(y2).ticks(5).tickSize(-width).tickPadding(6)).call(g2=>g2.select('.domain').remove()).call(g2=>g2.selectAll('.tick line').attr('stroke','var(--border)').attr('stroke-dasharray','2,3')).call(g2=>g2.selectAll('text').attr('fill','var(--text-secondary)').attr('font-size','7px'))\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Faceted Axes')\r
