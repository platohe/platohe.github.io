...
export const meta = {\r
  id: 'density-with-median',\r
  title: 'Density With Median',\r
  desc: 'Density With Median — a analysis chart visualization',\r
  category: 'Analysis',\r
  component: 'DensityWithMedian',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["analysis","density-with-median"],\r
}\r
\r
export default function DensityWithMedian({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    kdeAll.forEach((kde,i)=>{\r
      ...
      const m=d3.median(raw[i])||50\r
      g.append('line').attr('x1',x(m)).attr('x2',x(m)).attr('y1',y(kde.find(k=>Math.abs(k.v-m)<1)?.d||0)).attr('y2',y(0)).attr('stroke',colors[i]).attr('stroke-width',1).attr('stroke-dasharray','2,2')\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Density with Median')\r
  },[customData])\r
