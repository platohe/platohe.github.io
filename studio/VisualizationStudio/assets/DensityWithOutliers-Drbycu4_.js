...
export const meta = {\r
  id: 'density-with-outliers',\r
  title: 'Density With Outliers',\r
  desc: 'Density With Outliers — a analysis chart visualization',\r
  category: 'Analysis',\r
  component: 'DensityWithOutliers',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["analysis","density-with-outliers"],\r
}\r
\r
export default function DensityWithOutliers({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    kdeAll.forEach((kde,i)=>{\r
      ...
      raw[i].filter(v=>v<lo||v>hi).forEach(v=> g.append('circle').attr('cx',x(v)).attr('cy',y(kde.find(k=>Math.abs(k.v-v)<1)?.d||0)).attr('r',2.2).attr('fill','#ef4444').attr('stroke','var(--bg)').attr('stroke-width',0.6))\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Density with Outliers')\r
  },[customData])\r
