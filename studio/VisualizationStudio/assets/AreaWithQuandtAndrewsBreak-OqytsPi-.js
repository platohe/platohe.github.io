...
export const meta = {\r
  id: 'area-with-quandt-andrews-break',\r
  title: 'Area With Quandt Andrews Break',\r
  desc: 'Area With Quandt Andrews Break — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithQuandtAndrewsBreak',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-quandt-andrews-break"],\r
}\r
\r
export default function AreaWithQuandtAndrewsBreak({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Quandt-Andrews test for unknown break point\r
    const maxStat=10\r
    const breakPoint=25\r
    ...
    g.append('line').attr('x1',x(25)).attr('x2',x(25)).attr('y1',y(0)).attr('y2',y(80)).attr('stroke','#ef4444').attr('stroke-width',2).attr('stroke-dasharray','4,4')\r
    g.append('text').attr('x',x(25)).attr('y',10).attr('text-anchor','middle').attr('fill','#ef4444').attr('font-size','6px').attr('font-weight',600).text('Break (QL)')\r
    g.append('path').datum(curveData).attr('d',area).attr('fill',colors[0]).attr('fill-opacity',0.18).attr('stroke',colors[0]).attr('stroke-width',1.6)\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Quandt-Andrews Break')\r
