...
export const meta = {\r
  id: 'area-with-hodrick-prescott',\r
  title: 'Area With Hodrick Prescott',\r
  desc: 'Area With Hodrick Prescott — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithHodrickPrescott',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-hodrick-prescott"],\r
}\r
\r
export default function AreaWithHodrickPrescottFilter({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Hodrick-Prescott filter (simplified)\r
    const lambda=1600\r
    const n=curveData.length\r
    const trend=curveData.map((_,i)=>({\r
      x:curveData[i].x,\r
      y: curveData[i].y - (Math.random()-0.5)*2 // simplified HP filter residual\r
    }))\r
    ...
    // Trend line\r
    g.append('path').datum(curveData.map((d,i)=>({x:curveData[i].x, y:curveData[i].y - (Math.random()-0.5)*2}))).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1.4).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Hodrick-Prescott Filter')\r
