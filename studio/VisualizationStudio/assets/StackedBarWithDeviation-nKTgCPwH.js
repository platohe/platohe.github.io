...
export const meta = {\r
  id: 'stacked-bar-with-deviation',\r
  title: 'Stacked Bar With Deviation',\r
  desc: 'Stacked Bar With Deviation — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'StackedBarWithDeviation',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["bars","stacked-bar-with-deviation"],\r
}\r
\r
export default function StackedBarWithDeviation({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    const avg=d3.mean(data,d=>d.total)||30\r
    ...
    series.forEach(s=> g.selectAll(\`rect.\${s.key}\`).data(s).join('rect')\r
      ...
      .attr('fill',col(s.key)).attr('fill-opacity',(d)=> d.data.total>=avg?0.82:0.42).attr('stroke','var(--bg)').attr('rx',1))\r
    data.forEach(d=>{\r
      const dev=d.total-avg\r
      g.append('text').attr('x',(x(d.cat)??0)+x.bandwidth()/2).attr('y',y(d.total)-4).attr('text-anchor','middle').attr('fill',dev>=0?'#22c55e':'#ef4444').attr('font-size','6px').text(\`\${dev>=0?'+':''}\${dev.toFixed(0)}\`)\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Stacked Bar with Deviation')\r
  },[customData])\r
