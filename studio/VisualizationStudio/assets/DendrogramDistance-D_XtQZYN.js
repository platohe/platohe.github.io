...
// DendrogramDistance: Threshold cut line drawn.\r
export const meta = {\r
  id: 'dendrogram-distance',\r
  title: 'Dendrogram Distance',\r
  desc: 'Dendrogram Distance — a hierarchies chart visualization',\r
  category: 'Hierarchies',\r
  component: 'DendrogramDistance',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["hierarchies","dendrogram-distance"],\r
}\r
\r
export default function DendrogramDistance({ data: customData }) {\r
  ...
  useEffect(() => {\r
    ...
    const leafXs = root.leaves().map(l => px(l))\r
    const midX = (Math.min(...leafXs) + Math.max(...leafXs)) / 2\r
    g.append('line').attr('x1', midX).attr('x2', midX).attr('y1', 38).attr('y2', 250)\r
      .attr('stroke', '#ef4444').attr('stroke-dasharray', '4,3').attr('stroke-opacity', 0.7)\r
    g.append('text').attr('x', midX + 4).attr('y', 46).attr('font-size', '8px').attr('fill', '#ef4444').text('cut')\r
