...
export const meta = {\r
  id: 'area-with-cyclic-boosting',\r
  title: 'Area With Cyclic Boosting',\r
  desc: 'Area With Cyclic Boosting — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithCyclicBoosting',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-cyclic-boosting"],\r
}\r
\r
export default function AreaWithCyclicBoosting({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Cyclic boosting\r
    const cycles=5\r
    const cyclesData=Array.from({length:5},()=>curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*4})))\r
    cyclesData.forEach((cycle,i)=>{\r
      g.append('path').datum(cycle).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Cyclic Boosting')\r
