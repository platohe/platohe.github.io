...
export const meta = {\r
  id: 'area-with-range-selector',\r
  title: 'Area With Range Selector',\r
  desc: 'Area With Range Selector — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithRangeSelector',\r
  complexity: 'beginner',\r
  interactivity: ["brush"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-range-selector"],\r
}\r
\r
export default function AreaWithRangeSelector({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Range selector (brushable area at bottom)\r
    const brushHeight=30\r
    const brushG=svg.append('g').attr('transform',\`translate(\${margin.left},\${margin.top+height+10})\`)\r
    const x2=d3.scaleLinear().domain(d3.extent(curveData,d=>d.x)).range([0,width])\r
    const y2=d3.scaleLinear().domain([0,80]).range([brushHeight,0])\r
    const area2=d3.area().x(d=>x2(d.x)).y0(y2(0)).y1(d=>y2(d.y)).curve(d3.curveBasis)\r
    brushG.append('path').datum(curveData).attr('d',area2).attr('fill',colors[0]).attr('fill-opacity',0.3).attr('stroke',colors[0]).attr('stroke-width',0.8)\r
    brushG.append('rect').attr('width',width).attr('height',brushHeight).attr('fill','var(--bg)').attr('stroke','var(--border)')\r
    // Main area\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Range Selector')\r
