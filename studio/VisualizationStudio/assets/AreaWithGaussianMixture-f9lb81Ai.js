...
export const meta = {\r
  id: 'area-with-gaussian-mixture',\r
  title: 'Area With Gaussian Mixture',\r
  desc: 'Area With Gaussian Mixture — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithGaussianMixture',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-gaussian-mixture"],\r
}\r
\r
export default function AreaWithGaussianMixture({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Gaussian mixture (3 components)\r
    const mix=3\r
    const components=Array.from({length:3},()=>curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*6})))\r
    components.forEach((comp,i)=>{\r
      g.append('path').datum(comp).attr('d',d3.area().x(d=>x(d.x)).y0(y(42)).y1(d=>y(42+d.y)).curve(d3.curveBasis))\r
        .attr('fill',colors[i]).attr('fill-opacity',0.1).attr('stroke',colors[i]).attr('stroke-width',0.8)\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Gaussian Mixture')\r
