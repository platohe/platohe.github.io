...
export const meta = {\r
  id: 'bean-with-deviation',\r
  title: 'Bean With Deviation',\r
  desc: 'Bean With Deviation — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'BeanWithDeviation',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","bean-with-deviation"],\r
}\r
\r
export default function BeanWithDeviation({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    raw.forEach((vals,i)=>{\r
      ...
      g.append('line').attr('x1',cx-10).attr('x2',cx+10).attr('y1',y(avg)).attr('y2',y(avg)).attr('stroke',dev>=0?'#22c55e':'#ef4444').attr('stroke-width',1.4).attr('stroke-dasharray','2,2')\r
      g.append('circle').attr('cx',cx).attr('cy',y(avg)).attr('r',2).attr('fill',dev>=0?'#22c55e':'#ef4444')\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Bean with Deviation')\r
  },[customData])\r
