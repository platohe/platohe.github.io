...
export const meta = {\r
  id: 'raincloud-with-deviation',\r
  title: 'Raincloud With Deviation',\r
  desc: 'Raincloud With Deviation — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'RaincloudWithDeviation',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","raincloud-with-deviation"],\r
}\r
\r
export default function RaincloudWithDeviation({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    const avgs=groups.map(gr=> d3.mean(data.filter(d=>d.group===gr),d=>d.v)||50)\r
    const totalAvg=d3.mean(avgs)||50\r
    groups.forEach((gr,i)=>{\r
      ...
      g.append('path').datum(kde).attr('d',area).attr('fill',avgs[i]>=totalAvg?'#22c55e':'#ef4444').attr('fill-opacity',avgs[i]>=totalAvg?0.22:0.14).attr('stroke',avgs[i]>=totalAvg?'#22c55e':'#ef4444').attr('stroke-width',1)\r
      const avg=avgs[i]\r
      g.append('line').attr('x1',x(avg)).attr('x2',x(avg)).attr('y1',yy- h(kde.find(k=>Math.abs(k.v-avg)<1)?.d||0)).attr('y2',yy+6).attr('stroke',avg>=totalAvg?'#22c55e':'#ef4444').attr('stroke-width',1).attr('stroke-dasharray','2,2')\r
      vals.forEach(v=> g.append('circle').attr('cx',x(v)).attr('cy',yy+10+(Math.random()-0.5)*6).attr('r',1.6).attr('fill',avgs[i]>=totalAvg?'#22c55e':'#ef4444').attr('opacity',0.62))\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Raincloud with Deviation')\r
  },[customData])\r
