...
export const meta = {\r
  id: 'area-with-voronoi-overlay',\r
  title: 'Area With Voronoi Overlay',\r
  desc: 'Area With Voronoi Overlay — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithVoronoiOverlay',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-voronoi-overlay"],\r
}\r
\r
export default function AreaWithVoronoiOverlay({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Voronoi overlay\r
    const voronoi=d3.Delaunay.from(curveData,d=>x(d.x),d=>y(d.y)).voronoi([0,0,width,height])\r
    g.append('g').attr('stroke','#94a3b8').attr('stroke-width',0.5).attr('fill','none')\r
      .selectAll('path').data(curveData).enter().append('path')\r
      .attr('d',(d,i)=>voronoi.renderCell(i))\r
      .attr('fill',colors[0]).attr('fill-opacity',0.08).attr('stroke','#94a3b8').attr('stroke-width',0.5)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Voronoi Overlay')\r
