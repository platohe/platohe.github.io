...
\r
export const meta = {\r
  id: 'line-chart-stacked',\r
  title: 'Line Chart Stacked',\r
  desc: 'Line Chart Stacked — a lines chart visualization',\r
  category: 'Lines',\r
  component: 'LineChartStacked',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["lines","line-chart-stacked"],\r
}\r
\r
export default function LineChartStacked({ data: customData }) {\r
  const ref = useRef(null)\r
  const DEFAULT_DATA = [{"month":"Jan","A":30,"B":20},{"month":"Feb","A":40,"B":25},{"month":"Mar","A":55,"B":35},{"month":"Apr","A":50,"B":40},{"month":"May","A":65,"B":45},{"month":"Jun","A":70,"B":50}]\r
  useEffect(() => {\r
    const svg = d3.select(ref.current); svg.selectAll('*').remove()\r
    const data = (Array.isArray(customData) && customData.length > 0) ? customData : DEFAULT_DATA\r
    const keys = Object.keys(data[0]).filter(k => k !== 'month')\r
    const margin = { top: 25, right: 40, bottom: 35, left: 40 }\r
    ...
    const lg = svg.append('g').attr('transform', 'translate(' + (W - 75) + ',14)')\r
    keys.forEach((k, i) => {\r
