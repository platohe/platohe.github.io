...
// CityBlocks3d: CityBlocks3d — isometric pseudo-3D.\r
export const meta = {\r
  id: 'city-blocks3d',\r
  title: 'City Blocks3d',\r
  desc: 'City Blocks3d — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'CityBlocks3d',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","city-blocks3d"],\r
}\r
\r
export default function CityBlocks3d({ data: customData }) {\r
...
const shade = (hex, f) => { const n = parseInt(hex.slice(1), 16); const r = Math.min(255, ((n >> 16) & 255) * f), gg = Math.min(255, ((n >> 8) & 255) * f), b2 = Math.min(255, (n & 255) * f); return '#' + [r, gg, b2].map(v => Math.round(v).toString(16).padStart(2, '0')).join('') }\r
    ...
    let seedC=3; const rc=()=>{ seedC=(seedC*16807)%2147483647; return seedC/2147483647 }\r
    for(let bx=0;bx<6;bx++)for(let by2=0;by2<5;by2++){ if((bx+by2)%5===4) continue\r
     box(bx*24-70, by2*24-55, 0, 18, 18, 14+rc()*46, ['#6366f1','#94a3b8','#f59e0b','#10b981'][rc()>0.7?1:(bx+by2)%4===0?2:0]) }\r
  }, [customData])\r
