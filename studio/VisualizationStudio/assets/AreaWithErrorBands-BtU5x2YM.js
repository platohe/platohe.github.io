...
export const meta = {\r
  id: 'area-with-error-bands',\r
  title: 'Area With Error Bands',\r
  desc: 'Area With Error Bands — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithErrorBands',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-error-bands"],\r
}\r
\r
export default function AreaWithErrorBands({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Error bands\r
    const window=5\r
    const errors=curveData.map((d,i)=>{\r
      const slice=curveData.slice(Math.max(0,i-2),i+3)\r
      const vals=slice.map(d=>d.y)\r
      const mean=d3.mean(vals)||0\r
      const se=d3.deviation(vals)||0/Math.sqrt(slice.length)\r
      return {x:d.x, mean, upper:mean+1.96*se, lower:mean-1.96*se}\r
    })\r
    const area=d3.area().x(d=>x(d.x)).y0(d=>y(d.lower)).y1(d=>y(d.upper)).curve(d3.curveBasis)\r
    g.append('path').datum(errors).attr('d',area).attr('fill',colors[0]).attr('fill-opacity',0.14).attr('stroke','none')\r
    ...
    g.append('path').datum(errors).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.mean)).curve(d3.curveBasis)).attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1.2).attr('stroke-dasharray','3,2')\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Error Bands')\r
