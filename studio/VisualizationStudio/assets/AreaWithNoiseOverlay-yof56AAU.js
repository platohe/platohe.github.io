...
export const meta = {\r
  id: 'area-with-noise-overlay',\r
  title: 'Area With Noise Overlay',\r
  desc: 'Area With Noise Overlay — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithNoiseOverlay',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-noise-overlay"],\r
}\r
\r
export default function AreaWithNoiseOverlay({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Noise overlay\r
    const noisePoints=100\r
    const noiseData=Array.from({length:noisePoints},()=>({x:Math.random()*width,y:Math.random()*height}))\r
    g.selectAll('circle.noise').data(noiseData).enter().append('circle')\r
      .attr('cx',d=>d.x).attr('cy',d=>d.y).attr('r',1).attr('fill','#94a3b8').attr('opacity',0.15)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Noise Overlay')\r
