...
// ParallelCoordinatesLabeled: Values labeled at first and last axis.\r
export const meta = {\r
  id: 'parallel-coordinates-labeled',\r
  title: 'Parallel Coordinates Labeled',\r
  desc: 'Parallel Coordinates Labeled — a lines chart visualization',\r
  category: 'Lines',\r
  component: 'ParallelCoordinatesLabeled',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["lines","parallel-coordinates-labeled"],\r
}\r
\r
export default function ParallelCoordinatesLabeled({ data: customData }) {\r
...
\r
    ...
    data.rows.forEach((r, idx) => {\r
      ...
      const last = n - 1\r
      g.append('text').attr('x', xs(last) + 6).attr('y', pts[last][1] + 3).attr('font-size', '8px').attr('fill', 'var(--text-secondary)').text(r.name + ' · ' + r[data.headers[last]])\r
      g.append('text').attr('x', xs(0) - 6).attr('y', pts[0][1] + 3).attr('text-anchor', 'end').attr('font-size', '8px').attr('fill', 'var(--text-secondary)').text(r.name)\r
    })\r
    \r
    void data\r
  }, [customData])\r
