...
export const meta = {\r
  id: 'area-with-wild-bootstrap',\r
  title: 'Area With Wild Bootstrap',\r
  desc: 'Area With Wild Bootstrap — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithWildBootstrap',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-wild-bootstrap"],\r
}\r
\r
export default function AreaWithWildBootstrap({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Wild bootstrap\r
    const bootstrap=50\r
    const boots=Array.from({length:bootstrap},()=>{\r
      const weights=Array.from({length:curveData.length},()=>Math.random())\r
      const sum=d3.sum(weights)\r
      return curveData.map((d,i)=>({x:d.x, y:d.y*weights[i]/d3.mean(weights)}))\r
    })\r
    boots.forEach(b=>{\r
      g.append('path').datum(b).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis))\r
        .attr('fill','none').attr('stroke',colors[0]).attr('stroke-width',0.5).attr('opacity',0.1)\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Wild Bootstrap')\r
