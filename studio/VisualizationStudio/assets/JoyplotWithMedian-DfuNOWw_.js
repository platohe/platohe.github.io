...
export const meta = {\r
  id: 'joyplot-with-median',\r
  title: 'Joyplot With Median',\r
  desc: 'Joyplot With Median — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'JoyplotWithMedian',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","joyplot-with-median"],\r
}\r
\r
export default function JoyplotWithMedian({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    groups.forEach((gr,i)=>{\r
      ...
      g.append('line').attr('x1',x(m)).attr('x2',x(m)).attr('y1',yy- h(kde.find(k=>Math.abs(k.v-m)<1)?.d||0)).attr('y2',yy).attr('stroke','#0f172a').attr('stroke-width',1).attr('stroke-dasharray','2,2')\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Joyplot with Median')\r
  },[customData])\r
