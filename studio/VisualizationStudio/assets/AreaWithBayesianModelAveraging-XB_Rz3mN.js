...
export const meta = {\r
  id: 'area-with-bayesian-model-averaging',\r
  title: 'Area With Bayesian Model Averaging',\r
  desc: 'Area With Bayesian Model Averaging — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithBayesianModelAveraging',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-bayesian-model-averaging"],\r
}\r
\r
export default function AreaWithBayesianModelAveraging({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    const numModels=5\r
    const weights=[0.3,0.25,0.2,0.15,0.1]\r
    const models=curveData.map((_,i)=>curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*4})))\r
    const ensemble=curveData.map((_,i)=>({\r
      x:curveData[i].x,\r
      y:d3.mean(models.map(m=>m[i].y))\r
    }))\r
    models.forEach((m,i)=>{\r
      g.append('path').datum(models[i]).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Bayesian Model Averaging')\r
