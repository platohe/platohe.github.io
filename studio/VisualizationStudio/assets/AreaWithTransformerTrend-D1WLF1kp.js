...
export const meta = {\r
  id: 'area-with-transformer-trend',\r
  title: 'Area With Transformer Trend',\r
  desc: 'Area With Transformer Trend — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithTransformerTrend',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-transformer-trend"],\r
}\r
\r
export default function AreaWithTransformerTrend({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Transformer-style attention trend\r
    const seqLen=8\r
    ...
    // Attention weights as overlay\r
    g.selectAll('circle').data(attention).enter().append('circle')\r
      .attr('cx',d=>x(d.x)).attr('cy',d=>y(d.y)).attr('r',d=>4+d.attn*10)\r
      .attr('fill',colors[0]).attr('fill-opacity',d=>d.attn).attr('stroke','none')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Transformer Trend')\r
