...
export const meta = {\r
  id: 'ridge-with-points',\r
  title: 'Ridge With Points',\r
  desc: 'Ridge With Points — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'RidgeWithPoints',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","ridge-with-points"],\r
}\r
\r
export default function RidgeWithPoints({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    groups.forEach((gr,i)=>{\r
      ...
      // points at baseline jittered\r
      vals.forEach(v=> g.append('circle').attr('cx',x(v)).attr('cy',yy+4+(Math.random()-0.5)*6).attr('r',1.4).attr('fill',colors[i]).attr('opacity',0.48))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Ridge with Points')\r
  },[customData])\r
