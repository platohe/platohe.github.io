...
export const meta = {\r
  id: 'area-with-dpo',\r
  title: 'Area With D P O',\r
  desc: 'Area With D P O — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithDPO',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-d-p-o"],\r
}\r
\r
export default function AreaWithDPO({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // DPO (Direct Preference Optimization)\r
    const beta=0.1\r
    const chosen=curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*4}))\r
    const rejected=curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*2}))\r
    ...
    // Preferred vs rejected\r
    g.append('path').datum(chosen).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#22c55e').attr('stroke-width',1.4).attr('stroke-dasharray','3,2')\r
    g.append('path').datum(rejected).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#ef4444').attr('stroke-width',1).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with DPO')\r
