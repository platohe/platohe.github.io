...
// ProjectStatusDashboard: KPI tiles + mini charts composition.\r
export const meta = {\r
  id: 'project-status-dashboard',\r
  title: 'Project Status Dashboard',\r
  desc: 'Project Status Dashboard — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'ProjectStatusDashboard',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","project-status-dashboard"],\r
}\r
\r
export default function ProjectStatusDashboard({ data: customData }) {\r
  ...
  useEffect(() => {\r
    ...
    g.append('text').attr('x', 24).attr('y', 34).attr('font-size', '11px').attr('font-weight', 700)\r
      .attr('fill', 'var(--text-secondary)').text('ProjectStatus · This Week')\r
    const kpis = [["Done","142",16],["Open","87",-11],["Blocked","6",-25]]\r
    kpis.forEach((k, i) => {\r
      const x = 24 + i * 122\r
    ...
    const bars = [["Sprint",48],["Backlog",37],["QA",15]]\r
    let by = 178\r
    bars.forEach((b, i) => {\r
