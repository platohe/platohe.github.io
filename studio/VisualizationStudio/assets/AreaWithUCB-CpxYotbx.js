...
export const meta = {\r
  id: 'area-with-ucb',\r
  title: 'Area With U C B',\r
  desc: 'Area With U C B — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithUCB',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-u-c-b"],\r
}\r
\r
export default function AreaWithUpperConfidenceBound({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    const ucbData=curveData.map((d,i)=>{\r
      const ucb=values.map((v,i)=>v + Math.sqrt(2*Math.log(i+1)/(counts[i]+1)))\r
      return {x:d.x, ucb:d3.max(ucb)||0}\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with UCB')\r
