...
export const meta = {\r
  id: 'bean-with-median',\r
  title: 'Bean With Median',\r
  desc: 'Bean With Median — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'BeanWithMedian',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","bean-with-median"],\r
}\r
\r
export default function BeanWithMedian({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    raw.forEach((vals,i)=>{\r
      ...
      g.append('line').attr('x1',cx-12).attr('x2',cx+12).attr('y1',y(m)).attr('y2',y(m)).attr('stroke',colors[i]).attr('stroke-width',1.8)\r
      g.append('circle').attr('cx',cx).attr('cy',y(m)).attr('r',2.4).attr('fill',colors[i]).attr('stroke','#fff')\r
      // mean dot\r
      const mean=d3.mean(vals)||50\r
      g.append('line').attr('x1',cx-6).attr('x2',cx+6).attr('y1',y(mean)).attr('y2',y(mean)).attr('stroke','#0f172a').attr('stroke-width',1).attr('stroke-dasharray','2,2')\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Bean with Median')\r
  },[customData])\r
