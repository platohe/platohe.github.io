...
export const meta = {\r
  id: 'area-with-reward-modeling',\r
  title: 'Area With Reward Modeling',\r
  desc: 'Area With Reward Modeling — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithRewardModeling',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-reward-modeling"],\r
}\r
\r
export default function AreaWithRewardModeling({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Reward model\r
    const reward=curveData.map(d=>({x:d.x, y: -(d.y-42)*(d.y-42)/100 + 42}))\r
    ...
    g.append('path').datum(reward).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1.4).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Reward Modeling')\r
