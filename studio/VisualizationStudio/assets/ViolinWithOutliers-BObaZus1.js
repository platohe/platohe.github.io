...
export const meta = {\r
  id: 'violin-with-outliers',\r
  title: 'Violin With Outliers',\r
  desc: 'Violin With Outliers — a analysis chart visualization',\r
  category: 'Analysis',\r
  component: 'ViolinWithOutliers',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["analysis","violin-with-outliers"],\r
}\r
\r
export default function ViolinWithOutliers({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    groups.forEach((gr,i)=>{\r
      ...
      vals.filter(v=>v<lo||v>hi).forEach(v=> g.append('circle').attr('cx',cx+(Math.random()-0.5)*6).attr('cy',y(v)).attr('r',2.4).attr('fill','#ef4444').attr('stroke','var(--bg)'))\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Violin with Outliers')\r
  },[customData])\r
