...
export const meta = {\r
  id: 'sina-with-outliers',\r
  title: 'Sina With Outliers',\r
  desc: 'Sina With Outliers — a dots chart visualization',\r
  category: 'Dots',\r
  component: 'SinaWithOutliers',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["dots","sina-with-outliers"],\r
}\r
\r
export default function SinaWithOutliers({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    groups.forEach((gr,i)=>{\r
      ...
      vals.forEach(v=>{\r
        ...
        g.append('circle').attr('cx',(x(gr)??0)+x.bandwidth()/2 + j).attr('cy',y(v)).attr('r',isOut?3.2:2).attr('fill',isOut?'#ef4444':colors[i]).attr('fill-opacity',isOut?0.92:0.68).attr('stroke','var(--bg)').attr('stroke-width',isOut?0.8:0.4)\r
        if(isOut) g.append('text').attr('x',(x(gr)??0)+x.bandwidth()/2 + j).attr('y',y(v)-6).attr('text-anchor','middle').attr('fill','#ef4444').attr('font-size','5px').text('outlier')\r
      })\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Sina with Outliers')\r
  },[customData])\r
