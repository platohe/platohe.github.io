...
export const meta = {\r
  id: 'area-with-mirror-descent',\r
  title: 'Area With Mirror Descent',\r
  desc: 'Area With Mirror Descent — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithMirrorDescent',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-mirror-descent"],\r
}\r
\r
export default function AreaWithMirrorDescent({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    curveData.forEach(d=>{\r
      const grad=d.y-42\r
      const mirror=dual+0.1*grad\r
      const primal=Math.max(0,mirror-0.1)\r
      dual=mirror\r
      trend.push({x:d.x, y:primal})\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Mirror Descent')\r
