...
// ChordDiagramMinimal: Thin monochrome chords.\r
export const meta = {\r
  id: 'chord-diagram-minimal',\r
  title: 'Chord Diagram Minimal',\r
  desc: 'Chord Diagram Minimal — a networks chart visualization',\r
  category: 'Networks',\r
  component: 'ChordDiagramMinimal',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-shape"],\r
  tags: ["networks","chord-diagram-minimal"],\r
}\r
\r
export default function ChordDiagramMinimal({ data: customData }) {\r
  const ref = useRef(null)\r
  useEffect(() => {\r
    const svg = d3.select(ref.current); svg.selectAll('*').remove()\r
    const DEFAULT_DATA = { names: ["N","S","E","W"], matrix: [[0,10,6,4],[10,0,8,5],[6,8,0,7],[4,5,7,0]] }\r
...
\r
    ...
    chords.groups.forEach((group, i) => {\r
      svg.append('path').attr('d', arc(group)).attr('transform', tr)\r
        .attr('fill', color(data.names[i])).attr('fill-opacity', 0.35)\r
    ...
    flat.forEach(cd => {\r
      ...
      svg.append('path').attr('d', ribbon(cd)).attr('transform', tr)\r
        ...
        .attr('fill-opacity', 0.25 * (0.55 + 0.45*cd.source.value/maxV))\r
