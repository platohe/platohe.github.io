...
export const meta = {\r
  id: 'area-with-kalman-filter',\r
  title: 'Area With Kalman Filter',\r
  desc: 'Area With Kalman Filter — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithKalmanFilter',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-kalman-filter"],\r
}\r
\r
export default function AreaWithKalmanFilter({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    curveData.forEach(d=>{\r
      const K = P / (P + 5)\r
      const xPred = filtered[filtered.length-1]?.y || 42\r
      P = (1 - K) * (filtered[filtered.length-1]?.P || 10) + 0.1\r
      filtered.push({x: d.x, y: xPred + K * (d.y - xPred)})\r
    })\r
    ...
    // Kalman filter line\r
    g.append('path').datum(curveData.map((d,i)=>({x:d.x, y: d.y + (Math.random()-0.5)*2}))).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1.2).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Kalman Filter')\r
