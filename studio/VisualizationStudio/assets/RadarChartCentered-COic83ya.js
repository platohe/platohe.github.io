...
\r
export const meta = {\r
  id: 'radar-chart-centered',\r
  title: 'Radar Chart Centered',\r
  desc: 'Radar Chart Centered — a fun chart visualization',\r
  category: 'Fun',\r
  component: 'RadarChartCentered',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-shape"],\r
  tags: ["fun","radar-chart-centered"],\r
}\r
\r
export default function RadarChartCentered({ data: customData }) {\r
  const ref = useRef(null)\r
  const DEFAULT_DATA = {"labels":["Speed","Power","Endurance","Agility","Strength","Flexibility"],"values":[75,88,60,90,72,88]}\r
  useEffect(() => {\r
    ...
    [0.25, 0.5, 0.75, 1.0].forEach(level => {\r
      ...
      svg.append('polygon').attr('points', pts.map(p => p.join(',')).join(' ')).attr('fill', 'none').attr('stroke', 'var(--border)').attr('stroke-width', level === 1 ? 1.2 : 0.6).attr('stroke-opacity', level === 1 ? 0.5 : 0.25)\r
    })\r
    labels.forEach((label, i) => {\r
      ...
      svg.append('text').attr('x', cx + (r + 18) * Math.cos(a)).attr('y', cy + (r + 18) * Math.sin(a) + 3).attr('text-anchor', 'middle').attr('fill', 'var(--text-secondary)').attr('font-size', '8.5px').text(label)\r
    })\r
