...
export const meta = {\r
  id: 'area-with-streamline-ribbon',\r
  title: 'Area With Streamline Ribbon',\r
  desc: 'Area With Streamline Ribbon — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithStreamlineRibbon',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-streamline-ribbon"],\r
}\r
\r
export default function AreaWithStreamlineRibbon({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Streamline flow lines\r
    const streamlines=5\r
    for(let s=0;s<streamlines;s++){\r
      const offset=s*2-4\r
      const stream=curveData.map(d=>({x:d.x, y:d.y+offset}))\r
      g.append('path').datum(stream).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
        .attr('fill','none').attr('stroke',colors[s%colors.length]).attr('stroke-width',1).attr('opacity',0.5)\r
    }\r
    // Base area\r
    g.append('path').datum(curveData).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis))\r
      .attr('fill',colors[0]).attr('fill-opacity',0.14).attr('stroke',colors[0]).attr('stroke-width',1.6)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Streamline Ribbon')\r
