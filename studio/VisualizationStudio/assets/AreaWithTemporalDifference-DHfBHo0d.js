...
export const meta = {\r
  id: 'area-with-temporal-difference',\r
  title: 'Area With Temporal Difference',\r
  desc: 'Area With Temporal Difference — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithTemporalDifference',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-temporal-difference"],\r
}\r
\r
export default function AreaWithTemporalDifference({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // TD learning\r
    const alpha=0.1, gamma=0.9\r
    let V=42\r
    const tdData=[]\r
    curveData.forEach((d,i)=>{\r
      if(i>0){\r
        const reward=-(d.y-42)*(d.y-42)/100\r
        const td=d.y + 0.9*V - V\r
        V+=0.1*td\r
        tdData.push({x:d.x, td})\r
      }\r
    })\r
    ...
    g.append('path').datum(tdData).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.td*10)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1.2).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Temporal Difference')\r
