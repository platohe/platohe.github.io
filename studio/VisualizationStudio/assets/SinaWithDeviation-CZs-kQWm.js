...
export const meta = {\r
  id: 'sina-with-deviation',\r
  title: 'Sina With Deviation',\r
  desc: 'Sina With Deviation — a dots chart visualization',\r
  category: 'Dots',\r
  component: 'SinaWithDeviation',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["dots","sina-with-deviation"],\r
}\r
\r
export default function SinaWithDeviation({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    groups.forEach((gr,i)=>{\r
      ...
      vals.forEach(v=>{\r
        ...
        g.append('circle').attr('cx',(x(gr)??0)+x.bandwidth()/2 + j).attr('cy',y(v)).attr('r',2).attr('fill',avgs[i]>=totalAvg?'#22c55e':'#ef4444').attr('fill-opacity',avgs[i]>=totalAvg?0.72:0.42).attr('stroke','var(--bg)').attr('stroke-width',0.4)\r
      })\r
      const avg=avgs[i]\r
      g.append('line').attr('x1',(x(gr)??0)+x.bandwidth()/2-12).attr('x2',(x(gr)??0)+x.bandwidth()/2+12).attr('y1',y(avg)).attr('y2',y(avg)).attr('stroke',avg>=totalAvg?'#22c55e':'#ef4444').attr('stroke-width',1.2).attr('stroke-dasharray','2,2')\r
    })\r
    g.append('line').attr('x1',0).attr('x2',width).attr('y1',y(totalAvg)).attr('y2',y(totalAvg)).attr('stroke','#0f172a').attr('stroke-width',1).attr('stroke-dasharray','3,2').attr('opacity',0.42)\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Sina with Deviation')\r
  },[customData])\r
