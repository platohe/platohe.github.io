...
export const meta = {\r
  id: 'area-with-multiple-structural-breaks',\r
  title: 'Area With Multiple Structural Breaks',\r
  desc: 'Area With Multiple Structural Breaks — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithMultipleStructuralBreaks',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-multiple-structural-breaks"],\r
}\r
\r
export default function AreaWithBaiPerronBreaks({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Multiple break points (Bai-Perron)\r
    const breaks=[20, 35, 45]\r
    ...
    breaks.forEach(b=>{\r
      g.append('line').attr('x1',x(b)).attr('x2',x(b)).attr('y1',y(0)).attr('y2',y(80)).attr('stroke','#ef4444').attr('stroke-width',2).attr('stroke-dasharray','4,4')\r
      g.append('text').attr('x',x(b)).attr('y',10).attr('text-anchor','middle').attr('fill','#ef4444').attr('font-size','6px').attr('font-weight',600).text('Break')\r
    })\r
    g.append('path').datum(curveData).attr('d',area).attr('fill',colors[0]).attr('fill-opacity',0.18).attr('stroke',colors[0]).attr('stroke-width',1.6)\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Bai-Perron Breaks')\r
