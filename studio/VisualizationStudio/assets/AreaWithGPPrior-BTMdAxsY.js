...
export const meta = {\r
  id: 'area-with-gpprior',\r
  title: 'Area With G P Prior',\r
  desc: 'Area With G P Prior — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithGPPrior',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-g-p-prior"],\r
}\r
\r
export default function AreaWithGPrior({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // GP prior samples\r
    const priorSamples=Array.from({length:10},()=>curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*4})))\r
    // Prior samples\r
    priorSamples.forEach(s=>{\r
      g.append('path').datum(s).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
        .attr('fill','none').attr('stroke',colors[0]).attr('stroke-width',0.5).attr('opacity',0.3)\r
    })\r
    // Mean\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with GP Prior')\r
