...
export const meta = {\r
  id: 'histogram-with-median',\r
  title: 'Histogram With Median',\r
  desc: 'Histogram With Median — a analysis chart visualization',\r
  category: 'Analysis',\r
  component: 'HistogramWithMedian',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["analysis","histogram-with-median"],\r
}\r
\r
export default function HistogramWithMedian({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    g.append('line').attr('x1',x(m)).attr('x2',x(m)).attr('y1',0).attr('y2',height).attr('stroke','#0f172a').attr('stroke-width',1.4).attr('stroke-dasharray','3,2')\r
    g.append('circle').attr('cx',x(m)).attr('cy',8).attr('r',3).attr('fill','#0f172a')\r
    g.append('text').attr('x',x(m)+4).attr('y',11).attr('fill','#0f172a').attr('font-size','7px').text(\`median \${m.toFixed(1)}\`)\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Histogram with Median')\r
  },[customData])\r
