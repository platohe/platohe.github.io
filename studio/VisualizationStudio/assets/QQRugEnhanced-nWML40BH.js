...
export const meta = {\r
  id: 'qqrug-enhanced',\r
  title: 'Q Q Rug Enhanced',\r
  desc: 'Q Q Rug Enhanced — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'QQRugEnhanced',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","q-q-rug-enhanced"],\r
}\r
\r
export default function QQRugEnhanced({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    const y=d3.scaleLinear().domain(d3.extent(sorted) ).nice().range([height-14,0])\r
    const g=svg.append('g').attr('transform',\`translate(\${margin.left},\${margin.top})\`)\r
    g.append('g').attr('transform',\`translate(0,\${height-14})\`).call(d3.axisBottom(x).ticks(5).tickSize(0)).call(g2=>g2.selectAll('text').attr('fill','var(--text-secondary)').attr('font-size','7px'))\r
    ...
    sorted.forEach((v,i)=>{ g.append('line').attr('x1',x(exp[i])).attr('x2',x(exp[i])).attr('y1',height-14).attr('y2',height-10).attr('stroke',colors[0]).attr('opacity',0.48); g.append('line').attr('x1',-4).attr('x2',0).attr('y1',y(v)).attr('y2',y(v)).attr('stroke',colors[0]).attr('opacity',0.48) })\r
    g.append('text').attr('x',width/2).attr('y',height+8).attr('text-anchor','middle').attr('fill','var(--text-secondary)').attr('font-size','7px').text('Theoretical →')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Q-Q Rug Enhanced')\r
  },[customData])\r
