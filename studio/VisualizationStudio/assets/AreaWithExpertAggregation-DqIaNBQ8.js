...
export const meta = {\r
  id: 'area-with-expert-aggregation',\r
  title: 'Area With Expert Aggregation',\r
  desc: 'Area With Expert Aggregation — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithExpertAggregation',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-expert-aggregation"],\r
}\r
\r
export default function AreaWithExpertAggregation({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Expert aggregation with Cook's distance weighting\r
    const expertsData=Array.from({length:5},()=>curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*4})))\r
    const weights=[0.3,0.25,0.2,0.15,0.1]\r
    const ensemble=curveData.map((_,i)=>({\r
      x:curveData[i].x,\r
      y:d3.sum(expertsData.map((e,j)=>e[i].y*weights[j]))\r
    }))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Expert Aggregation')\r
