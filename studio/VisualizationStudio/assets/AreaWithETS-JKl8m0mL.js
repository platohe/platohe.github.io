...
export const meta = {\r
  id: 'area-with-ets',\r
  title: 'Area With E T S',\r
  desc: 'Area With E T S — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithETS',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-e-t-s"],\r
}\r
\r
export default function AreaWithETS({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // ETS (Error, Trend, Seasonal) - simplified\r
    const alpha=0.3, beta=0.1, gamma=0.1, period=12\r
    let level=curveData[0].y, trend=0\r
    const seasonal=Array(12).fill(0)\r
    const fitted=[]\r
    curveData.forEach((d,i)=>{\r
      const sIdx=i%12\r
      const prevLevel=fitted[fitted.length-1]?.level||curveData[0].y\r
      const prevTrend=fitted[fitted.length-1]?.trend||0\r
      const level=alpha*(d.y-seasonal[sIdx])+(1-alpha)*(prevLevel+prevTrend)\r
      const trend=beta*(level-fitted[fitted.length-1]?.level||0)+(1-beta)*prevTrend\r
      const seasonalVal=gamma*(d.y-level)+(1-gamma)*seasonal[sIdx]\r
      fitted.push({level, trend, seasonal:seasonalVal})\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with ETS')\r
