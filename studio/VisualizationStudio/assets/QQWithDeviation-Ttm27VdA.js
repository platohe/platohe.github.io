...
export const meta = {\r
  id: 'qqwith-deviation',\r
  title: 'Q Q With Deviation',\r
  desc: 'Q Q With Deviation — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'QQWithDeviation',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","q-q-with-deviation"],\r
}\r
\r
export default function QQWithDeviation({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    const avgExp=d3.mean(exp)||50, avgObs=d3.mean(sorted)||50\r
    const dev=avgObs-avgExp\r
    g.append('line').attr('x1',x(avgExp-5)).attr('x2',x(avgExp+5)).attr('y1',y(avgObs)).attr('y2',y(avgObs)).attr('stroke',dev>=0?'#22c55e':'#ef4444').attr('stroke-width',1.2).attr('stroke-dasharray','2,2')\r
    g.append('text').attr('x',x(avgExp)+6).attr('y',y(avgObs)-6).attr('fill',dev>=0?'#22c55e':'#ef4444').attr('font-size','7px').text(\`\${dev>=0?'+':''}\${dev.toFixed(1)}\`)\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Q-Q with Deviation')\r
  },[customData])\r
