...
export default function HistogramDensity({ data: customData }) {\r
  ...
  useEffect(() => {\r
    ...
    const hist = d3.histogram().domain([0, 100]).thresholds(20)(data)\r
    ...
    g.selectAll('rect').data(hist).join('rect').attr('x', d => x(d.x0) + 1).attr('y', d => y(d.length)).attr('width', d => Math.max(1, x(d.x1) - x(d.x0) - 2)).attr('height', d => h - y(d.length)).attr('fill', colors[0]).attr('opacity', 0.6).attr('rx', 1)\r
    const kdeData = kde(x.domain(), data)\r
    if (kdeData.length > 1) {\r
      const line = d3.line().x(d => x(d.x)).y(d => y(d.density * d3.max(hist, b => b.length) * 0.9)).curve(d3.curveBasis)\r
      g.append('path').datum(kdeData).attr('d', line).attr('fill', 'none').attr('stroke', colors[0]).attr('stroke-width', 2).attr('opacity', 0.8)\r
    }\r
