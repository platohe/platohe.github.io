...
export const meta = {\r
  id: 'area-with-self-exciting-process',\r
  title: 'Area With Self Exciting Process',\r
  desc: 'Area With Self Exciting Process — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithSelfExcitingProcess',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-self-exciting-process"],\r
}\r
\r
export default function AreaWithSelfExcitingProcess({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Self-exciting process intensity\r
    const mu=0.1, alpha=0.5, beta=1.0\r
    let lambda=0.1\r
    const intensity=curveData.map((d,i)=>{\r
      if(i===0) return {x:d.x, lambda:0.1}\r
      const dt=d.x-curveData[i-1].x\r
      lambda=0.1 + (lambda-0.1)*Math.exp(-beta*dt) + alpha*Math.exp(-beta*dt)\r
      return {x:d.x, lambda}\r
    })\r
    // Intensity overlay\r
    g.append('path').datum(intensity).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.lambda*20)).curve(d3.curveBasis))\r
      .attr('fill','#ef4444').attr('fill-opacity',0.12).attr('stroke','none')\r
    // Original area\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Self-Exciting Process')\r
