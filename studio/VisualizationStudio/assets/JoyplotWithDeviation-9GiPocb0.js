...
export const meta = {\r
  id: 'joyplot-with-deviation',\r
  title: 'Joyplot With Deviation',\r
  desc: 'Joyplot With Deviation — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'JoyplotWithDeviation',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","joyplot-with-deviation"],\r
}\r
\r
export default function JoyplotWithDeviation({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    groups.forEach((gr,i)=>{\r
      ...
      const avg=avgs[i]\r
      const dev=avg-totalAvg\r
      g.append('line').attr('x1',x(avg)).attr('x2',x(avg)).attr('y1',yy- h(kde.find(k=>Math.abs(k.v-avg)<1)?.d||0)).attr('y2',yy).attr('stroke',dev>=0?'#22c55e':'#ef4444').attr('stroke-width',1)\r
      g.append('text').attr('x',-6).attr('y',yy+3).attr('text-anchor','end').attr('fill','var(--text)').attr('font-size','7px').attr('font-weight',600).text(\`\${gr} \${dev>=0?'+':''}\${dev.toFixed(1)}\`)\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Joyplot with Deviation')\r
  },[customData])\r
