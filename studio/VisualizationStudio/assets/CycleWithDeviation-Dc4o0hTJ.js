...
export const meta = {\r
  id: 'cycle-with-deviation',\r
  title: 'Cycle With Deviation',\r
  desc: 'Cycle With Deviation — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'CycleWithDeviation',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","cycle-with-deviation"],\r
}\r
\r
export default function CycleWithDeviation({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    const avg=d3.mean(data,d=>d.v)||32\r
    ...
    data.forEach(d=>{\r
      const dev=d.v-avg\r
      g.append('rect').attr('x',x(d.month)??0).attr('y',y(d.v)).attr('width',x.bandwidth()).attr('height',height-y(d.v)).attr('fill',dev>=0?'#22c55e':'#ef4444').attr('fill-opacity',dev>=0?0.72:0.42).attr('stroke',dev>=0?'#22c55e':'#ef4444').attr('rx',2)\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Cycle with Deviation')\r
  },[customData])\r
