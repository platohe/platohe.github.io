...
export const meta = {\r
  id: 'area-with-confidence-interval',\r
  title: 'Area With Confidence Interval',\r
  desc: 'Area With Confidence Interval — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithConfidenceInterval',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-confidence-interval"],\r
}\r
\r
export default function AreaWithConfidenceInterval({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    const window=20\r
    const bands=curveData.map((d,i)=>{\r
      const slice=curveData.slice(Math.max(0,i-19),i+1)\r
      const vals=slice.map(d=>d.y)\r
      const mean=d3.mean(vals)||0\r
      const std=d3.deviation(vals)||0\r
      return {x:d.x, mean, upper:mean+1.96*std, lower:mean-1.96*std}\r
    })\r
    const area=d3.area().x(d=>x(d.x)).y0(d=>y(d.upper)).y1(d=>y(d.lower)).curve(d3.curveBasis)\r
    g.append('path').datum(bands).attr('d',area).attr('fill',colors[0]).attr('fill-opacity',0.12).attr('stroke',colors[0]).attr('stroke-width',1)\r
    ...
    g.append('path').datum(bands).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.mean)).curve(d3.curveBasis)).attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1).attr('stroke-dasharray','3,2')\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Confidence Interval')\r
