...
export const meta = {\r
  id: 'grouped-bar-with-avg',\r
  title: 'Grouped Bar With Avg',\r
  desc: 'Grouped Bar With Avg — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'GroupedBarWithAvg',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["bars","grouped-bar-with-avg"],\r
}\r
\r
export default function GroupedBarWithAvg({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    // avg line per group\r
    groups.forEach(gp=>{\r
      const vals=data.filter(d=>d.group===gp).map(d=>d.v)\r
      const avg=d3.mean(vals)||0\r
      const xStart=x0(gp)??0, xEnd=(x0(gp)??0)+x0.bandwidth()\r
      g.append('line').attr('x1',xStart).attr('x2',xEnd).attr('y1',y(avg)).attr('y2',y(avg)).attr('stroke','#0f172a').attr('stroke-width',1.2).attr('stroke-dasharray','3,2')\r
      g.append('text').attr('x',xEnd-2).attr('y',y(avg)-4).attr('text-anchor','end').attr('fill','#0f172a').attr('font-size','6px').text(avg.toFixed(1))\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Grouped Bar with Avg')\r
  },[customData])\r
