...
export const meta = {\r
  id: 'area-with-layered-shadows',\r
  title: 'Area With Layered Shadows',\r
  desc: 'Area With Layered Shadows — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithLayeredShadows',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-layered-shadows"],\r
}\r
\r
export default function AreaWithLayeredShadows({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Multiple layered shadows\r
    for(let i=3;i>=1;i--){\r
      g.append('path').datum(curveData).attr('d',area).attr('fill',colors[0]).attr('fill-opacity',0.05*i).attr('stroke',colors[0]).attr('stroke-width',1+0.3*i).attr('transform',\`translate(\${1.5*i},\${1.5*i})\`)\r
    }\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Layered Shadows')\r
