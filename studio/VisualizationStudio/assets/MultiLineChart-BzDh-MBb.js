...
\r
export const meta = {\r
  id: 'multi-line-chart',\r
  title: 'Multi Line Chart',\r
  desc: 'Multi Line Chart — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'MultiLineChart',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["bars","multi-line-chart"],\r
}\r
\r
export default function MultiLineChart({ data: customData }) {\r
...
\r
    // Multi-series time series data\r
...
\r
    const y = d3.scaleLinear()\r
      .domain([0, d3.max(data, d => d3.max(series, s => d[s]))])\r
      .range([height, 0])\r
\r
...
\r
    // Draw lines for each series\r
    series.forEach((s, i) => {\r
      const lineData = data.map(d => ({ date: d.date, value: d[s] }))\r
\r
      // Area fill\r
...
\r
      // Dots\r
      g.selectAll(\`circle-\${i}\`)\r
        .data(lineData)\r
        .join('circle')\r
        .attr('cx', d => x(d.date))\r
...
\r
    ...
    svg.append('text')\r
      ...
      .text('Multi-Series Line Chart')\r
\r
  }, [customData])\r
\r
