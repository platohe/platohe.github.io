...
export const meta = {\r
  id: 'area-with-population-based-training',\r
  title: 'Area With Population Based Training',\r
  desc: 'Area With Population Based Training — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithPopulationBasedTraining',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-population-based-training"],\r
}\r
\r
export default function AreaWithPopulationBasedTraining({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Population based training\r
    const population=10\r
    const generations=5\r
    const populationsData=Array.from({length:generations},()=>curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*4})))\r
    populationsData.forEach((gen,i)=>{\r
      g.append('path').datum(gen).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Population Based Training')\r
