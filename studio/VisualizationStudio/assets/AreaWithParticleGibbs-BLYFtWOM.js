...
export const meta = {\r
  id: 'area-with-particle-gibbs',\r
  title: 'Area With Particle Gibbs',\r
  desc: 'Area With Particle Gibbs — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithParticleGibbs',\r
  complexity: 'beginner',\r
  interactivity: ["animate"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-particle-gibbs"],\r
}\r
\r
export default function AreaWithParticleGibbs({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Particle Gibbs sampler\r
    const particles=50\r
    let particlesData=Array.from({length:50},()=>({x:0, y:42}))\r
    curveData.forEach((d,i)=>{\r
      particlesData.forEach(p=>{\r
        p.x += (Math.random()-0.5)*2\r
        p.y += (Math.random()-0.5)*2\r
      })\r
      // Weight by likelihood\r
      particlesData.forEach(p=>{\r
        const diff=Math.abs(p.y - d.y)\r
        p.weight = Math.exp(-0.5*Math.pow(diff/5,2))\r
      })\r
      // Normalize\r
      const sum=d3.sum(particlesData, p=>p.weight)\r
      particlesData.forEach(p=> p.weight/=sum)\r
      // Resample\r
      const newParticles=[]\r
      for(let i=0;i<50;i++){\r
        const r=Math.random()\r
        let cum=0, idx=0\r
        while(cum<r && idx<50){ cum+=particlesData[idx].weight; idx++ }\r
        newParticles.push({...particlesData[Math.min(idx,49)]})\r
      }\r
      particlesData=newParticles\r
      const mean=d3.mean(particlesData, p=>p.y)||42\r
      g.append('circle').attr('cx',x(d.x)).attr('cy',y(mean)).attr('r',1.5).attr('fill','#22c55e').attr('opacity',0.5)\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Particle Gibbs')\r
