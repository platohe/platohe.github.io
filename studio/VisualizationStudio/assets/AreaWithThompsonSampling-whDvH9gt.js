var e=`import { useEffect, useRef, useMemo } from 'react'\r
import * as d3 from 'd3'\r
import { W, H, colors } from './utils'\r
import { makeBaseCurve } from './areaParams'\r
export const meta = {\r
  id: 'area-with-thompson-sampling',\r
  title: 'Area With Thompson Sampling',\r
  desc: 'Area With Thompson Sampling — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithThompsonSampling',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-thompson-sampling"],\r
}\r
\r
export default function AreaWithThompsonSampling({ data: customData, onCurveData }) {\r
  const ref=useRef(null)\r
  const isCurveData = Array.isArray(customData) && customData.length > 0 && customData[0].x != null\r
  const params = useMemo(() => ({\r
    formula: 'sinusoidal',\r
    amp: customData?.amp ?? 15,\r
    offset: customData?.offset ?? 40,\r
    freq: customData?.freq ?? 2,\r
    points: customData?.points ?? 60,\r
  }), [customData])\r
\r
  const curveData = useMemo(() => isCurveData ? customData : makeBaseCurve(params), [isCurveData, params, customData])\r
  useEffect(()=>{\r
    if (onCurveData) onCurveData(curveData)\r
    const svg=d3.select(ref.current); svg.selectAll('*').remove()\r
    const margin={top:28,right:14,bottom:24,left:36}\r
    const width=W-margin.left-margin.right, height=H-margin.top-margin.bottom\r
    const x=d3.scaleLinear().domain(d3.extent(curveData,d=>d.x)).range([0,width])\r
    const y=d3.scaleLinear().domain([0,80]).range([height,0])\r
    const g=svg.append('g').attr('transform',\`translate(\${margin.left},\${margin.top})\`)\r
    // Thompson sampling bandit\r
    const arms=3\r
    const alphas=[1,1,1], betas=[1,1,1]\r
    const armsData=Array.from({length:3},()=>curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*4})))\r
    g.append('path').datum(curveData).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis))\r
      .attr('fill',colors[0]).attr('fill-opacity',0.18).attr('stroke',colors[0]).attr('stroke-width',1.6)\r
    const posteriors=[0,0,0]\r
    curveData.forEach((d,i)=>{\r
      const betaSample=(a,b)=>{ const u1=Math.max(1e-9,Math.random()), u2=Math.max(1e-9,Math.random()); const x1=Math.pow(u1,1/a), x2=Math.pow(u2,1/b); return x1/(x1+x2) }\r
      const samples=Array.from({length:3},(_,j)=>betaSample(1+alphas[j],1+betas[j]))\r
      const chosen=d3.scan(samples, (a,b)=>b-a)\r
      posteriors[chosen]++\r
      alphas[chosen]++\r
      betas[chosen]++\r
      g.append('rect').attr('x',x(d.x)).attr('y',y(d.y)).attr('width',width/3).attr('height',height-y(d.y))\r
        .attr('fill',colors[chosen]).attr('fill-opacity',0.3).attr('stroke','none')\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Thompson Sampling')\r
  },[params, curveData, onCurveData])\r
  return <svg ref={ref} width={W} height={H} viewBox={\`0 0 \${W} \${H}\`} />\r
}`;export{e as default};