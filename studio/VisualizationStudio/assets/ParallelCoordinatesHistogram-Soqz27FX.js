...
// ParallelCoordinatesHistogram: Density bands along each axis.\r
export const meta = {\r
  id: 'parallel-coordinates-histogram',\r
  title: 'Parallel Coordinates Histogram',\r
  desc: 'Parallel Coordinates Histogram — a lines chart visualization',\r
  category: 'Lines',\r
  component: 'ParallelCoordinatesHistogram',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["lines","parallel-coordinates-histogram"],\r
}\r
\r
export default function ParallelCoordinatesHistogram({ data: customData }) {\r
...
\r
    ...
    ;data.headers.forEach((h, i) => {\r
      const bins = {}\r
      rows2.forEach(r => { const b = Math.floor((y0 - 8 - scales[i](r[h])) / 24); bins[b] = (bins[b] || 0) + 1 })\r
      Object.entries(bins).forEach(([b, cnt]) => {\r
        g.append('rect').attr('x', xs(i) - 14).attr('y', y0 - 32 - (+b) * 24).attr('width', 28).attr('height', 20)\r
          .attr('rx', 3).attr('fill', '#6366f1').attr('fill-opacity', 0.07 + 0.05 * Math.min(cnt, 5))\r
      })\r
    })\r
    void data\r
  }, [customData])\r
