...
export const meta = {\r
  id: 'area-with-bayesian-online-change-point',\r
  title: 'Area With Bayesian Online Change Point',\r
  desc: 'Area With Bayesian Online Change Point — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithBayesianOnlineChangePoint',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-bayesian-online-change-point"],\r
}\r
\r
export default function AreaWithBayesianOnlineChangePoint({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Bayesian online change point detection\r
    const runLength=0\r
    const cpProbs=[]\r
    let runLen=0\r
    curveData.forEach((d,i)=>{\r
      if(i>0){\r
        const prob=Math.exp(-Math.pow(d.y-curveData[i-1].y,2)/50)\r
        if(Math.random()<prob*0.3){\r
          runLen=0\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Bayesian Online Change Point')\r
