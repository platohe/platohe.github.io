...
export const meta = {\r
  id: 'area-with-vmd',\r
  title: 'Area With V M D',\r
  desc: 'Area With V M D — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithVMD',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-v-m-d"],\r
}\r
\r
export default function AreaWithVMD({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Variational Mode Decomposition (simplified)\r
    const modes=3\r
    const modesData=Array.from({length:3},()=>curveData.map(d=>({x:d.x, y:d.y*(Math.random()*0.3+0.1)})))\r
    // Plot VMD modes\r
    [0,1,2].forEach(m=>{\r
      g.append('path').datum(curveData.map((d,i)=>({x:d.x, y:42+curveData[i].y*(Math.random()*0.2)})))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Variational Mode Decomposition')\r
