...
export const meta = {\r
  id: 'area-with-dirichlet-process',\r
  title: 'Area With Dirichlet Process',\r
  desc: 'Area With Dirichlet Process — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithDirichletProcess',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-dirichlet-process"],\r
}\r
\r
export default function AreaWithDirichletProcess({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Cluster components\r
    [0,1,2].forEach(c=>{\r
      g.append('path').datum(curveData.map((d,i)=>({x:curveData[i].x, y:42+Math.random()*10-5})))\r
        .attr('d',d3.area().x(d=>x(d.x)).y0(y(42)).y1(d=>y(42+d.y)).curve(d3.curveBasis))\r
        .attr('fill',colors[c]).attr('fill-opacity',0.1).attr('stroke',colors[c]).attr('stroke-width',0.8)\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Dirichlet Process')\r
