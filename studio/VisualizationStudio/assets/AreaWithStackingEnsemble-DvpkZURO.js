...
export const meta = {\r
  id: 'area-with-stacking-ensemble',\r
  title: 'Area With Stacking Ensemble',\r
  desc: 'Area With Stacking Ensemble — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithStackingEnsemble',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-stacking-ensemble"],\r
}\r
\r
export default function AreaWithStackingEnsemble({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Stacking ensemble of trends\r
    const methods=5\r
    const trends=Array.from({length:5},()=>curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*4})))\r
    const ensemble=curveData.map((_,i)=>({x:curveData[i].x, y:d3.mean(trends.map(t=>t[i].y))}))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Stacking Ensemble')\r
