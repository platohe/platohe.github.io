...
export const meta = {\r
  id: 'bar-cascade',\r
  title: 'Bar Cascade',\r
  desc: 'Bar Cascade — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'BarCascade',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis","d3-shape"],\r
  tags: ["bars","bar-cascade"],\r
}\r
\r
export default function BarCascade({ data: customData }) {\r
...
\r
    ...
    if(!data||!data.length) data=Array.from({ length: 10 }, (_, i) => 20 + i * 8 + Math.random() * 20)\r
...
\r
    const bars = bg.selectAll('rect').data(data).join('rect')\r
...
\r
    const timer = d3.timer(elapsed => {\r
      ...
      bars.each((d, i, nodes) => {\r
        ...
        const phase = (t + i * 0.3) % (Math.PI * 2)\r
        const fill = (Math.sin(phase) + 1) / 2\r
        const hRaw = dv * fill\r
