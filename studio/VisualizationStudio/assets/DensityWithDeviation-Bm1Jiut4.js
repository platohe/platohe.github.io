...
export const meta = {\r
  id: 'density-with-deviation',\r
  title: 'Density With Deviation',\r
  desc: 'Density With Deviation — a analysis chart visualization',\r
  category: 'Analysis',\r
  component: 'DensityWithDeviation',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["analysis","density-with-deviation"],\r
}\r
\r
export default function DensityWithDeviation({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    kdeAll.forEach((kde,i)=>{\r
      ...
      g.append('line').attr('x1',x(avg)).attr('x2',x(avg)).attr('y1',y(kde.find(k=>Math.abs(k.v-avg)<1)?.d||0)).attr('y2',y(0)).attr('stroke',dev>=0?'#22c55e':'#ef4444').attr('stroke-width',1).attr('stroke-dasharray','2,2')\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Density with Deviation')\r
  },[customData])\r
