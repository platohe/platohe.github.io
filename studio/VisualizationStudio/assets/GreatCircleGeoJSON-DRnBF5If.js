var e=`import { useEffect, useRef } from 'react'\r
import * as d3 from 'd3'\r
import { W, H, colors } from './utils'\r
export const meta = {\r
  id: 'great-circle-geo-json',\r
  title: 'Great Circle Geo J S O N',\r
  desc: 'Great Circle Geo J S O N — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'GreatCircleGeoJSON',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","great-circle-geo-j-s-o-n"],\r
}\r
\r
export default function GreatCircleGeoJSON({ data: customData }) {\r
  const ref = useRef(null)\r
  useEffect(() => {\r
    const svg = d3.select(ref.current); svg.selectAll('*').remove()\r
    const DEFAULT_DATA = {"type":"FeatureCollection","features":[{"type":"Feature","properties":{"value":42},"geometry":{"type":"Polygon","coordinates":[[[-100,30],[-90,30],[-90,40],[-100,40],[-100,30]]]}},{"type":"Feature","properties":{"value":68},"geometry":{"type":"Polygon","coordinates":[[[-80,30],[-70,30],[-70,40],[-80,40],[-80,30]]]}},{"type":"Feature","properties":{"value":55},"geometry":{"type":"Polygon","coordinates":[[[-100,15],[-90,15],[-90,25],[-100,25],[-100,15]]]}},{"type":"Feature","properties":{"value":30},"geometry":{"type":"Polygon","coordinates":[[[-80,15],[-70,15],[-70,25],[-80,25],[-80,15]]]}}]}\r
    const data = (customData && customData.type === 'FeatureCollection') ? customData : DEFAULT_DATA\r
    const proj = d3.geoMercator().fitSize([W-40, H-40], data)\r
    const path = d3.geoPath().projection(proj)\r
    const cents = data.features.map(f=>({ f, c: path.centroid(f), v: f.properties.value }))\r
    const g = svg.append('g')\r
    const cScale = d3.scaleSequential(d3.interpolateBlues).domain([0, Math.max(...cents.map(q=>q.v))])\r
\r
    const interp=d3.geoInterpolate([-60,20],[120,45])\r
    const arcPts=d3.range(25).map(t=>proj(interp(t)))\r
    g.append('path').attr('d','M'+arcPts.join('L')).attr('fill','none').attr('stroke',colors[3]).attr('stroke-width',2.2).attr('stroke-dasharray','1,4')\r
    cents.forEach(q=>{ g.append('circle').attr('cx',q.c[0]).attr('cy',q.c[1]).attr('r',3).attr('fill',colors[0]) })\r
    g.append('text').attr('x',arcPts[Math.floor(arcPts.length/2)][0]).attr('y',arcPts[Math.floor(arcPts.length/2)][1]-8).attr('text-anchor','middle').attr('fill','var(--text-secondary)').attr('font-size','8px').text('great circle')\r
\r
  }, [customData])\r
  return <svg ref={ref} width={W} height={H} viewBox="0 0 400 300" />\r
}\r
`;export{e as default};