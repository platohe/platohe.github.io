...
export const meta = {\r
  id: 'area-with-pattern-fill',\r
  title: 'Area With Pattern Fill',\r
  desc: 'Area With Pattern Fill — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithPatternFill',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-pattern-fill"],\r
}\r
\r
export default function AreaWithPatternFill({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    const pattern=defs.append('pattern').attr('id','areaPattern').attr('patternUnits','userSpaceOnUse').attr('width',10).attr('height',10)\r
    pattern.append('rect').attr('width',10).attr('height',10).attr('fill',colors[0]).attr('fill-opacity',0.3)\r
    pattern.append('line').attr('x1',0).attr('y1',0).attr('x2',10).attr('y2',10).attr('stroke','#fff').attr('stroke-width',0.5).attr('opacity',0.3)\r
    pattern.append('line').attr('x1',10).attr('y1',0).attr('x2',0).attr('y2',10).attr('stroke','#fff').attr('stroke-width',0.5).attr('opacity',0.3)\r
    const area=d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)\r
    g.append('path').datum(curveData).attr('d',area).attr('fill','url(#areaPattern)').attr('stroke',colors[0]).attr('stroke-width',1.6)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Pattern Fill')\r
