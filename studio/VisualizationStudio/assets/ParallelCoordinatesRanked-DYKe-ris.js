...
// ParallelCoordinatesRanked: Sorted by total score, top highlighted.\r
export const meta = {\r
  id: 'parallel-coordinates-ranked',\r
  title: 'Parallel Coordinates Ranked',\r
  desc: 'Parallel Coordinates Ranked — a lines chart visualization',\r
  category: 'Lines',\r
  component: 'ParallelCoordinatesRanked',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["lines","parallel-coordinates-ranked"],\r
}\r
\r
export default function ParallelCoordinatesRanked({ data: customData }) {\r
...
\r
    ...
    const rankOrder = [...data.rows.keys()].sort((a, b) => d3.sum(data.headers, h => data.rows[b][h]) - d3.sum(data.headers, h => data.rows[a][h]))\r
    data.rows.forEach((r, idx) => {\r
      const bestIdx = rankOrder[rankOrder.length - 1]; const c = idx === bestIdx ? '#10b981' : colors[idx % colors.length]\r
      const pts = data.headers.map((h, hi) => [xs(hi), scales[hi](r[h])])\r
      g.append('path').attr('d', d3.line()(pts)).attr('fill', 'none')\r
        .attr('stroke', c).attr('stroke-width', (idx === rankOrder[rankOrder.length - 1] ? 2.6 : 1.4))\r
