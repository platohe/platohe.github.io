...
export const meta = {\r
  id: 'area-with-tab-net-attention',\r
  title: 'Area With Tab Net Attention',\r
  desc: 'Area With Tab Net Attention — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithTabNetAttention',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-tab-net-attention"],\r
}\r
\r
export default function AreaWithTabNetTrend({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // TabNet-style attention\r
    const steps=3\r
    const attentionSteps=Array.from({length:3},()=>curveData.map(d=>({x:d.x, attn:Math.exp(-Math.abs(d.x-26)/10)})))\r
    ...
    // Attention masks\r
    attentionSteps.forEach((step,i)=>{\r
      g.append('path').datum(step).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.attn*10)).curve(d3.curveBasis))\r
        .attr('fill','none').attr('stroke',colors[i]).attr('stroke-width',1).attr('opacity',0.5)\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with TabNet Attention')\r
