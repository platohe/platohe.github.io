...
export const meta = {\r
  id: 'area-with-bollinger',\r
  title: 'Area With Bollinger',\r
  desc: 'Area With Bollinger — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithBollinger',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-bollinger"],\r
}\r
\r
export default function AreaWithBollinger({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    const bands=curveData.map((d,i)=>{\r
      const slice=curveData.slice(Math.max(0,i-window+1),i+1)\r
      const vals=slice.map(d=>d.y)\r
      const mean=d3.mean(vals)||0\r
      const std=d3.deviation(vals)||0\r
      return {x:d.x, mean, upper:mean+2*std, lower:mean-2*std}\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Bollinger Bands')\r
