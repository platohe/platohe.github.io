...
export const meta = {\r
  id: 'area-with-phase-type-distribution',\r
  title: 'Area With Phase Type Distribution',\r
  desc: 'Area With Phase Type Distribution — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithPhaseTypeDistribution',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-phase-type-distribution"],\r
}\r
\r
export default function AreaWithPhaseTypeDistribution({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Phase-type distribution\r
    const phases=3\r
    const phaseData=Array.from({length:3},()=>curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*8})))\r
    ;[0,1,2].forEach(p=>{\r
      g.append('path').datum(phaseData[p]).attr('d',d3.area().x(d=>x(d.x)).y0(y(42)).y1(d=>y(42+d.y)).curve(d3.curveBasis))\r
        .attr('fill',colors[p]).attr('fill-opacity',0.1).attr('stroke',colors[p]).attr('stroke-width',0.8)\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Phase-Type Distribution')\r
