...
export const meta = {\r
  id: 'area-with-deviation',\r
  title: 'Area With Deviation',\r
  desc: 'Area With Deviation — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithDeviation',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-deviation"],\r
}\r
\r
export default function AreaWithDeviation({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    const svg=d3.select(ref.current); svg.selectAll('*').remove()\r
            const avg=d3.mean(curveData,d=>d.y)||42\r
    ...
    g.append('path').datum(curveData).attr('d',area).attr('fill',colors[0]).attr('fill-opacity',0.18).attr('stroke','none')\r
    ...
    curveData.forEach(d=>{\r
      const dev=d.y-avg\r
      g.append('circle').attr('cx',x(d.x)).attr('cy',y(d.y)).attr('r',2).attr('fill',dev>=0?'#22c55e':'#ef4444').attr('fill-opacity',dev>=0?0.72:0.42).attr('stroke','var(--bg)').attr('stroke-width',0.4)\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Deviation')\r
