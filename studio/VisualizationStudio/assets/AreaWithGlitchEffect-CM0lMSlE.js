...
export const meta = {\r
  id: 'area-with-glitch-effect',\r
  title: 'Area With Glitch Effect',\r
  desc: 'Area With Glitch Effect — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithGlitchEffect',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-glitch-effect"],\r
}\r
\r
export default function AreaWithGlitchEffect({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Glitch effect: multiple offset copies\r
    for(let i=0;i<3;i++){\r
      const offset=(Math.random()-0.5)*4\r
      g.append('path').datum(curveData).attr('d',area).attr('fill',colors[0]).attr('fill-opacity',0.05).attr('stroke',colors[0]).attr('stroke-width',1).attr('transform',\`translate(\${offset},\${(Math.random()-0.5)*2})\`)\r
    }\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Glitch Effect')\r
