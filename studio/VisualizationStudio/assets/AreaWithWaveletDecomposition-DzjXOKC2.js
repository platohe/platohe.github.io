...
export const meta = {\r
  id: 'area-with-wavelet-decomposition',\r
  title: 'Area With Wavelet Decomposition',\r
  desc: 'Area With Wavelet Decomposition — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithWaveletDecomposition',\r
  complexity: 'beginner',\r
  interactivity: ["animate"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-wavelet-decomposition"],\r
}\r
\r
export default function AreaWithWaveletDecomposition({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Haar wavelet levels\r
    const levels=4\r
    for(let level=0;level<levels;level++){\r
      const detail=curveData.map((d,i)=>({x:d.x, y:d.y + (Math.random()-0.5)*(10/(level+1))}))\r
      g.append('path').datum(detail).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Wavelet Decomposition')\r
