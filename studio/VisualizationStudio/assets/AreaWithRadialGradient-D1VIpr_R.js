...
export const meta = {\r
  id: 'area-with-radial-gradient',\r
  title: 'Area With Radial Gradient',\r
  desc: 'Area With Radial Gradient — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithRadialGradient',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-radial-gradient"],\r
}\r
\r
export default function AreaWithRadialGradient({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    const grad=defs.append('radialGradient').attr('id','areaRadialGrad').attr('cx','50%').attr('cy','50%').attr('r','50%')\r
    grad.append('stop').attr('offset','0%').attr('stop-color',colors[0]).attr('stop-opacity',0.8)\r
    grad.append('stop').attr('offset','50%').attr('stop-color',colors[1]).attr('stop-opacity',0.4)\r
    grad.append('stop').attr('offset','100%').attr('stop-color',colors[2]).attr('stop-opacity',0.1)\r
    const area=d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)\r
    g.append('path').datum(curveData).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)).attr('fill','url(#areaRadialGrad)').attr('stroke',colors[0]).attr('stroke-width',1.6)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Radial Gradient')\r
