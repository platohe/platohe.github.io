...
export const meta = {\r
  id: 'ppwith-median',\r
  title: 'P P With Median',\r
  desc: 'P P With Median — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'PPWithMedian',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","p-p-with-median"],\r
}\r
\r
export default function PPWithMedian({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    series.forEach((s,i)=>{\r
      ...
      const mIdx=Math.floor(sorted.length/2), mTh=pts[mIdx].th, mEmp=pts[mIdx].emp\r
      g.append('circle').attr('cx',x(mTh)).attr('cy',y(mEmp)).attr('r',3).attr('fill',colors[i]).attr('stroke','var(--bg)')\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('P-P with Median')\r
  },[customData])\r
