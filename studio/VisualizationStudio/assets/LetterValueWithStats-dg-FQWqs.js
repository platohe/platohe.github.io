...
export const meta = {\r
  id: 'letter-value-with-stats',\r
  title: 'Letter Value With Stats',\r
  desc: 'Letter Value With Stats — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'LetterValueWithStats',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","letter-value-with-stats"],\r
}\r
\r
export default function LetterValueWithStats({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    raw.forEach((vals,i)=>{\r
      ...
      g.append('text').attr('x',cx).attr('y',y(m)-8).attr('text-anchor','middle').attr('fill','#0f172a').attr('font-size','6px').text(m.toFixed(0))\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Letter-Value with Stats')\r
  },[customData])\r
