...
export const meta = {\r
  id: 'area-with-threshold-fill',\r
  title: 'Area With Threshold Fill',\r
  desc: 'Area With Threshold Fill — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithThresholdFill',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-threshold-fill"],\r
}\r
\r
export default function AreaWithThresholdFill({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Threshold-based fill\r
    const threshold=50\r
    const areaAbove=d3.area().x(d=>x(d.x)).y0(y(50)).y1(d=>y(Math.max(d.y,50))).curve(d3.curveBasis)\r
    const areaBelow=d3.area().x(d=>x(d.x)).y0(y(50)).y1(d=>y(Math.min(d.y,50))).curve(d3.curveBasis)\r
    ...
    g.append('line').attr('x1',0).attr('x2',width).attr('y1',y(50)).attr('y2',y(50)).attr('stroke','#f59e0b').attr('stroke-width',1).attr('stroke-dasharray','3,2')\r
    g.append('path').datum(curveData).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)).attr('fill','none').attr('stroke',colors[0]).attr('stroke-width',1.8)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Threshold Fill')\r
