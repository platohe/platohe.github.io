...
export const meta = {\r
  id: 'area-with-arrow-annotations',\r
  title: 'Area With Arrow Annotations',\r
  desc: 'Area With Arrow Annotations — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithArrowAnnotations',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-arrow-annotations"],\r
}\r
\r
export default function AreaWithArrowAnnotations({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Find peaks\r
    const peaks=curveData.filter((d,i)=>i>0&&i<curveData.length-1&&d.y>curveData[i-1].y&&d.y>curveData[i+1].y)\r
    ...
    // Arrow annotations at peaks\r
    peaks.forEach(p=>{\r
      const px=x(p.x), py=y(p.y)\r
      g.append('path').attr('d',\`M\${px},\${py-10} L\${px},\${py-20} L\${px+5},\${py-15} L\${px-5},\${py-15}Z\`).attr('fill','#ef4444')\r
      g.append('text').attr('x',px).attr('y',py-24).attr('text-anchor','middle').attr('fill','#ef4444').attr('font-size','7px').attr('font-weight',600).text(\`Peak \${p.y.toFixed(0)}\`)\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Arrow Annotations')\r
