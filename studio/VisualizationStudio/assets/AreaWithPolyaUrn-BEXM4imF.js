...
export const meta = {\r
  id: 'area-with-polya-urn',\r
  title: 'Area With Polya Urn',\r
  desc: 'Area With Polya Urn — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithPolyaUrn',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-polya-urn"],\r
}\r
\r
export default function AreaWithPolyaUrn({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    for(let i=0;i<draws;i++){\r
      const total=red+blue\r
      const drawRed=Math.random()<red/(red+blue)\r
      if(drawRed){ red++ } else { blue++ }\r
      drawsData.push({x:i, y:drawRed?55:25})\r
    }\r
    ...
    // Polya urn draws\r
    drawsData.forEach((d,i)=>{\r
      g.append('circle').attr('cx',x(d)).attr('cy',y(d.y)).attr('r',3).attr('fill',d.y>42?'#ef4444':'#3b82f6').attr('opacity',0.7)\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Polya Urn')\r
