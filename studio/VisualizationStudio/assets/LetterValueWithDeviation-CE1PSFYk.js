...
export default function LetterValueWithDeviation({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    const avgs=raw.map(vals=> d3.mean(vals)||50)\r
    ...
    raw.forEach((vals,i)=>{\r
      ...
      const avg=avgs[i], dev=avg-totalAvg\r
      g.append('circle').attr('cx',cx).attr('cy',y(avg)).attr('r',2).attr('fill',dev>=0?'#22c55e':'#ef4444').attr('stroke','var(--bg)')\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Letter-Value with Deviation')\r
  },[customData])\r
