...
export const meta = {\r
  id: 'density-with-avg',\r
  title: 'Density With Avg',\r
  desc: 'Density With Avg — a analysis chart visualization',\r
  category: 'Analysis',\r
  component: 'DensityWithAvg',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["analysis","density-with-avg"],\r
}\r
\r
export default function DensityWithAvg({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    kdeAll.forEach((kde,i)=>{\r
      ...
      const avg=d3.mean(raw[i])||50\r
      g.append('line').attr('x1',x(avg)).attr('x2',x(avg)).attr('y1',y(kde.find(k=>Math.abs(k.v-avg)<1)?.d||0)).attr('y2',y(0)).attr('stroke','#0f172a').attr('stroke-width',1).attr('stroke-dasharray','2,2')\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Density with Avg')\r
  },[customData])\r
