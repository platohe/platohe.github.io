...
export const meta = {\r
  id: 'area-with-contour-lines',\r
  title: 'Area With Contour Lines',\r
  desc: 'Area With Contour Lines — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithContourLines',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-contour-lines"],\r
}\r
\r
export default function AreaWithContourLines({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    const contourArea=d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)\r
    // Contour lines at specific levels\r
    const levels=[20,30,40,50,60,70]\r
    levels.forEach(level=>{\r
      g.append('path').datum(curveData).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(level)).curve(d3.curveBasis))\r
        .attr('fill','none').attr('stroke','#94a3b8').attr('stroke-width',0.8).attr('stroke-dasharray','2,2').attr('opacity',0.6)\r
      g.append('text').attr('x',width-4).attr('y',y(level)-4).attr('text-anchor','end').attr('fill','#94a3b8').attr('font-size','6px').text(level)\r
    })\r
    g.append('path').datum(curveData).attr('d',contourArea).attr('fill',colors[0]).attr('fill-opacity',0.18).attr('stroke',colors[0]).attr('stroke-width',1.6)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Contour Lines')\r
