...
export const meta = {\r
  id: 'area-with-random-forest-trend',\r
  title: 'Area With Random Forest Trend',\r
  desc: 'Area With Random Forest Trend — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithRandomForestTrend',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-random-forest-trend"],\r
}\r
\r
export default function AreaWithRandomForestTrend({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Random forest trend (simplified - ensemble of trees)\r
    const trees=5\r
    const predictions=[]\r
    for(let t=0;t<trees;t++){\r
      const pred=curveData.map((d,i)=>({x:d.x, y: d.y + (Math.random()-0.5)*4}))\r
      predictions.push(pred)\r
    }\r
    const ensemble=curveData.map((_,i)=>({\r
      x:curveData[i].x,\r
      y:d3.mean(predictions.map(p=>p[i].y))||curveData[i].y\r
    }))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Random Forest Trend')\r
