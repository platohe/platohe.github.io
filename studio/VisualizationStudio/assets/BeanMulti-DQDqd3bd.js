...
export const meta = {\r
  id: 'bean-multi',\r
  title: 'Bean Multi',\r
  desc: 'Bean Multi — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'BeanMulti',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","bean-multi"],\r
}\r
\r
export default function BeanMulti({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    raw.forEach((vals,i)=>{\r
      ...
      const left=d3.area().x0(d=>cx - w(d.d)).x1(()=>cx).y(d=>y(d.v)).curve(d3.curveBasis)\r
      const right=d3.area().x0(()=>cx).x1(d=>cx + w(d.d)).y(d=>y(d.v)).curve(d3.curveBasis)\r
      g.append('path').datum(kde).attr('d',left).attr('fill',colors[i]).attr('fill-opacity',0.18).attr('stroke',colors[i])\r
      g.append('path').datum(kde).attr('d',right).attr('fill',colors[i]).attr('fill-opacity',0.18).attr('stroke',colors[i])\r
      ...
      // rug inside\r
      vals.slice(0,18).forEach(v=> g.append('line').attr('x1',cx-2).attr('x2',cx+2).attr('y1',y(v)).attr('y2',y(v)).attr('stroke',colors[i]).attr('stroke-width',0.6).attr('opacity',0.42))\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Bean Multi')\r
  },[customData])\r
