...
export const meta = {\r
  id: 'footprint-chart-ohlc',\r
  title: 'Footprint Chart O H L C',\r
  desc: 'Footprint Chart O H L C — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'FootprintChartOHLC',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","footprint-chart-o-h-l-c"],\r
}\r
\r
export default function FootprintChartOHLC({ data: customData }) {\r
  ...
  useEffect(() => {\r
    ...
    pts.forEach((d,i)=>{\r
      ...
      g.append('rect').attr('x',x(d.date)).attr('y',y(Math.max(d.open,d.close))).attr('width',bw).attr('height',Math.max(1,Math.abs(y(d.open)-y(d.close)))).attr('fill',col).attr('opacity',0.7)\r
      g.append('rect').attr('x',x(d.date)+bw*0.2).attr('y',H-M.bottom+2).attr('width',bw*0.6).attr('height', (d.volume||(30+i*5))*0.4).attr('fill',col).attr('opacity',0.4)\r
      \r
      \r
    })\r
    g.append('text').attr('x',W/2).attr('y',M.top-4).attr('text-anchor','middle').attr('fill','var(--text-secondary)').attr('font-size','7px').text('FootprintChartOHLC')\r
  }, [customData])\r
