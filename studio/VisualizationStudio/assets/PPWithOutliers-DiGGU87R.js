...
export const meta = {\r
  id: 'ppwith-outliers',\r
  title: 'P P With Outliers',\r
  desc: 'P P With Outliers — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'PPWithOutliers',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","p-p-with-outliers"],\r
}\r
\r
export default function PPWithOutliers({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    const valsAll=series.flatMap(s=>s.vals)\r
    ...
    series.forEach((s,i)=>{\r
      ...
      const isOut=sorted.some(v=>v<lo||v>hi)\r
      g.append('path').datum(pts).attr('d',d3.line().x(d=>x(d.th)).y(d=>y(d.emp)).curve(d3.curveBasis)).attr('fill','none').attr('stroke',isOut?'#ef4444':colors[i]).attr('stroke-width',isOut?2:1.6).attr('opacity',isOut?0.92:0.82)\r
      pts.filter(p=>p.th*40+50<lo||p.th*40+50>hi).forEach(p=> g.append('circle').attr('cx',x(p.th)).attr('cy',y(p.emp)).attr('r',2.4).attr('fill','#ef4444').attr('stroke','var(--bg)'))\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('P-P with Outliers')\r
  },[customData])\r
