...
export const meta = {\r
  id: 'area-with-gaussian-process',\r
  title: 'Area With Gaussian Process',\r
  desc: 'Area With Gaussian Process — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithGaussianProcess',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-gaussian-process"],\r
}\r
\r
export default function AreaWithGaussianProcess({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Gaussian Process regression (simplified RBF kernel)\r
    // GP predictions with uncertainty\r
    const gpPreds=curveData.map((d,i)=>({\r
      x:d.x,\r
      mean:d.y + (Math.random()-0.5)*2,\r
      lower:d.y-3,\r
      upper:d.y+3\r
    }))\r
    ...
    // GP confidence band\r
    const gpBand=d3.area().x(d=>x(d.x)).y0(d=>y(d.lower)).y1(d=>y(d.upper)).curve(d3.curveBasis)\r
    g.append('path').datum(gpPreds).attr('d',gpBand).attr('fill','#f59e0b').attr('fill-opacity',0.1)\r
    ...
    // GP mean\r
    g.append('path').datum(gpPreds).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.mean)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1.4).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Gaussian Process')\r
