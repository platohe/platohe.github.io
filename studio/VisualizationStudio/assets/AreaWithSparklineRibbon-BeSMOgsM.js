...
export const meta = {\r
  id: 'area-with-sparkline-ribbon',\r
  title: 'Area With Sparkline Ribbon',\r
  desc: 'Area With Sparkline Ribbon — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithSparklineRibbon',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-sparkline-ribbon"],\r
}\r
\r
export default function AreaWithSparklineRibbon({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Multiple small sparkline areas\r
    const slices=4\r
    const sliceLen=Math.ceil(curveData.length/slices)\r
    for(let s=0;s<slices;s++){\r
      const slice=curveData.slice(s*sliceLen,(s+1)*sliceLen)\r
      const area=d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)\r
      g.append('path').datum(slice).attr('d',area).attr('fill',colors[s%colors.length]).attr('fill-opacity',0.18).attr('stroke',colors[s%colors.length]).attr('stroke-width',1.2)\r
    }\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Sparkline Ribbon')\r
