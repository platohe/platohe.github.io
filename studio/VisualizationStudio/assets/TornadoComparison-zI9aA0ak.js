...
export const meta = {\r
  id: 'tornado-comparison',\r
  title: 'Tornado Comparison',\r
  desc: 'Tornado Comparison — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'TornadoComparison',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","tornado-comparison"],\r
}\r
\r
export default function TornadoComparison({ data: customData }) {\r
  const ref=useRef(null)\r
  useEffect(()=>{\r
    const svg=d3.select(ref.current); svg.selectAll('*').remove()\r
    const DEFAULT=[{cat:'Revenue',low:-12,high:18},{cat:'Cost',low:-9,high:14},{cat:'Demand',low:-7,high:11},{cat:'Price',low:-5,high:9}]\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Tornado Comparison')\r
  },[customData])\r
