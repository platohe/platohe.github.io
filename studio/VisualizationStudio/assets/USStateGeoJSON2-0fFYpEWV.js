...
export const meta = {\r
  id: 'usstate-geo-json2',\r
  title: 'U S State Geo J S O N2',\r
  desc: 'U S State Geo J S O N2 — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'USStateGeoJSON2',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","u-s-state-geo-j-s-o-n2"],\r
}\r
\r
export default function USStateGeoJSON2({ data: customData }) {\r
  ...
  useEffect(() => {\r
    ...
    const proj = d3.geoEqualEarth().fitSize([W-40, H-40], data)\r
    ...
    g.selectAll('circle').data(data.features).join('circle').attr('cx',d=>path.centroid(d)[0]).attr('cy',d=>path.centroid(d)[1]).attr('r',d=>4+ d.properties.value*0.15).attr('fill',colors[5]).attr('opacity',0.6)\r
    \r
    g.append('text').attr('x',W/2).attr('y',H-8).attr('text-anchor','middle').attr('fill','var(--text-secondary)').attr('font-size','7px').text('USStateGeoJSON2 - geoEqualEarth')\r
  }, [customData])\r
