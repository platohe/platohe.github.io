...
export const meta = {\r
  id: 'box-with-outliers',\r
  title: 'Box With Outliers',\r
  desc: 'Box With Outliers — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'BoxWithOutliers',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","box-with-outliers"],\r
}\r
\r
export default function BoxWithOutliers({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    raw.forEach((vals,i)=>{\r
      ...
      const q1i=d3.quantile(sorted,0.25)||0, m=d3.median(sorted)||0, q3i=d3.quantile(sorted,0.75)||0, min=d3.min(sorted)||0, max=d3.max(sorted)||0\r
      const cx=(x(groups[i])??0)+x.bandwidth()/2, bw=x.bandwidth()*0.48\r
      g.append('line').attr('x1',cx).attr('x2',cx).attr('y1',y(min)).attr('y2',y(q1i)).attr('stroke',colors[i]).attr('stroke-width',1)\r
      g.append('line').attr('x1',cx).attr('x2',cx).attr('y1',y(q3i)).attr('y2',y(max)).attr('stroke',colors[i]).attr('stroke-width',1)\r
      g.append('rect').attr('x',cx-bw/2).attr('y',y(q3i)).attr('width',bw).attr('height',Math.max(2, y(q1i)-y(q3i))).attr('fill',colors[i]).attr('fill-opacity',0.22).attr('stroke',colors[i]).attr('stroke-width',1)\r
      g.append('line').attr('x1',cx-bw/2).attr('x2',cx+bw/2).attr('y1',y(m)).attr('y2',y(m)).attr('stroke',colors[i]).attr('stroke-width',1.6)\r
      vals.filter(v=>v<lo||v>hi).forEach(v=> g.append('circle').attr('cx',cx+(Math.random()-0.5)*10).attr('cy',y(v)).attr('r',2.2).attr('fill','#ef4444').attr('stroke','var(--bg)'))\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Box with Outliers')\r
  },[customData])\r
