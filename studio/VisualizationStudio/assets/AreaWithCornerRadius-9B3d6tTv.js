...
export const meta = {\r
  id: 'area-with-corner-radius',\r
  title: 'Area With Corner Radius',\r
  desc: 'Area With Corner Radius — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithCornerRadius',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-corner-radius"],\r
}\r
\r
export default function AreaWithCornerRadius({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Rounded corners using path interpolation\r
    ...
    // Rounded top edge\r
    const topPath=d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis)\r
    g.append('path').datum(curveData).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveCardinal.tension(0.8))).attr('fill','none').attr('stroke',colors[0]).attr('stroke-width',1).attr('stroke-dasharray','3,2').attr('opacity',0.5)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Corner Radius')\r
