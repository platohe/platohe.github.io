...
// EnergyUsageDashboard: KPI tiles + mini charts composition.\r
export const meta = {\r
  id: 'energy-usage-dashboard',\r
  title: 'Energy Usage Dashboard',\r
  desc: 'Energy Usage Dashboard — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'EnergyUsageDashboard',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","energy-usage-dashboard"],\r
}\r
\r
export default function EnergyUsageDashboard({ data: customData }) {\r
  ...
  useEffect(() => {\r
    ...
    g.append('text').attr('x', 24).attr('y', 34).attr('font-size', '11px').attr('font-weight', 700)\r
      .attr('fill', 'var(--text-secondary)').text('EnergyUsage · This Week')\r
    const kpis = [["Load","412MW",-3],["Solar","96MW",14],["Cost","$0.09/kWh",-2]]\r
    kpis.forEach((k, i) => {\r
      const x = 24 + i * 122\r
    ...
    const bars = [["Grid",58],["Renew",30],["Backup",12]]\r
    let by = 178\r
    bars.forEach((b, i) => {\r
