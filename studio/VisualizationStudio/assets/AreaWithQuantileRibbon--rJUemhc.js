...
export const meta = {\r
  id: 'area-with-quantile-ribbon',\r
  title: 'Area With Quantile Ribbon',\r
  desc: 'Area With Quantile Ribbon — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithQuantileRibbon',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-quantile-ribbon"],\r
}\r
\r
export default function AreaWithQuantileRibbon({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Quantile ribbon (10-90 percentile)\r
    const q10=8, q90=72\r
    const ribbon=d3.area().x(d=>x(d.x)).y0(d=>y(q90)).y1(d=>y(q10)).curve(d3.curveBasis)\r
    g.append('path').datum(curveData).attr('d',ribbon).attr('fill',colors[0]).attr('fill-opacity',0.14).attr('stroke','none')\r
    g.append('path').datum(curveData).attr('d',d3.area().x(d=>x(d.x)).y0(y(72)).y1(d=>y(8)).curve(d3.curveBasis)).attr('fill',colors[0]).attr('fill-opacity',0.08).attr('stroke','none')\r
    g.append('path').datum(curveData).attr('d',d3.area().x(d=>x(d.x)).y0(y(75)).y1(d=>y(25)).curve(d3.curveBasis)).attr('fill',colors[0]).attr('fill-opacity',0.05).attr('stroke','none')\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Quantile Ribbon')\r
