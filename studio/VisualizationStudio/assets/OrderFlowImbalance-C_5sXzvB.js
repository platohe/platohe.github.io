...
export const meta = {\r
  id: 'order-flow-imbalance',\r
  title: 'Order Flow Imbalance',\r
  desc: 'Order Flow Imbalance — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'OrderFlowImbalance',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","order-flow-imbalance"],\r
}\r
\r
export default function OrderFlowImbalance({ data: customData }) {\r
...
\r
    const zero=H-M.bottom+6, bh=18\r
    pts.forEach(d=>{\r
      const buy=d.close>=d.open?Math.abs(d.close-d.open)+1:0\r
      const sell=d.close<d.open?Math.abs(d.close-d.open)+1:0\r
      const tot=buy+sell||1\r
      const bw=x.bandwidth()\r
      g.append('rect').attr('x',x(d.date)).attr('y',zero).attr('width',bw*(buy/tot)).attr('height',bh*buy/tot+4).attr('fill','#10b981').attr('opacity',0.8)\r
      g.append('rect').attr('x',x(d.date)+bw*(buy/tot)).attr('y',zero).attr('width',bw*(sell/tot)).attr('height',bh*sell/tot+4).attr('fill','#ef4444').attr('opacity',0.8)\r
    })\r
        pts.forEach(d=>{\r
