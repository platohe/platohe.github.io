...
export const meta = {\r
  id: 'area-with-prophet-forecast',\r
  title: 'Area With Prophet Forecast',\r
  desc: 'Area With Prophet Forecast — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithProphetForecast',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-prophet-forecast"],\r
}\r
\r
export default function AreaWithProphetForecast({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Prophet-style forecast\r
    const trend=curveData.map((d,i)=>({x:d.x, y: d.y + (i*0.2)}))\r
    const seasonal=curveData.map((d,i)=>Math.sin(i*0.5)*2)\r
    const forecast=Array.from({length:10},(_,i)=>({\r
      x:52+i, y: curveData[51].y + (i+1)*0.2 + Math.sin((52+i)*0.5)*2\r
    }))\r
    ...
    g.append('path').datum(forecast).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis))\r
      .attr('fill','#ef4444').attr('fill-opacity',0.1).attr('stroke','#ef4444').attr('stroke-width',1).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Prophet Forecast')\r
