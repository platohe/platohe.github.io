...
export const meta = {\r
  id: 'area-with-momentum-trend',\r
  title: 'Area With Momentum Trend',\r
  desc: 'Area With Momentum Trend — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithMomentumTrend',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-momentum-trend"],\r
}\r
\r
export default function AreaWithMomentumTrend({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Momentum trend\r
    const momentum=curveData.map((d,i)=>({x:d.x, y: d.y + (i>0?(curveData[i].y-curveData[i-1].y)*2:0)}))\r
    ...
    g.append('path').datum(momentum).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1.4).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Momentum Trend')\r
