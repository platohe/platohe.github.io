...
export const meta = {\r
  id: 'area-with-dot-grid-overlay',\r
  title: 'Area With Dot Grid Overlay',\r
  desc: 'Area With Dot Grid Overlay — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithDotGridOverlay',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-dot-grid-overlay"],\r
}\r
\r
export default function AreaWithDotGridOverlay({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Dot grid overlay\r
    const cols=10, rows=8\r
    for(let r=0;r<rows;r++){\r
      for(let c=0;c<cols;c++){\r
        g.append('circle').attr('cx',c*width/(cols-1)).attr('cy',r*height/(rows-1)).attr('r',2).attr('fill','var(--border)').attr('opacity',0.15)\r
      }\r
    }\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Dot Grid Overlay')\r
