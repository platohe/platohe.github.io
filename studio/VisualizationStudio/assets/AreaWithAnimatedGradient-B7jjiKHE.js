...
export const meta = {\r
  id: 'area-with-animated-gradient',\r
  title: 'Area With Animated Gradient',\r
  desc: 'Area With Animated Gradient — a animation chart visualization',\r
  category: 'Animation',\r
  component: 'AreaWithAnimatedGradient',\r
  complexity: 'beginner',\r
  interactivity: ["animate"],\r
  d3Api: ["d3-scale","d3-axis","d3-transition"],\r
  tags: ["animation","area-with-animated-gradient"],\r
}\r
\r
export default function AreaWithAnimatedGradient({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    const grad=defs.append('linearGradient').attr('id','animatedAreaGrad').attr('x1','0%').attr('y1','0%').attr('x2','100%').attr('y2','0%')\r
    for(let i=0;i<=10;i++){\r
      grad.append('stop').attr('offset',\`\${i*10}%\`).attr('stop-color',colors[i%colors.length]).attr('stop-opacity',0.8-i*0.05)\r
    }\r
    // Animate gradient\r
    let offset=0\r
    const animate=()=>{\r
      offset=(offset+0.5)%100\r
      grad.selectAll('stop').attr('offset',(d,i)=>\`\${(i*10+offset)%100}%\`)\r
      requestAnimationFrame(animate)\r
    }\r
    animate()\r
    const area=d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)\r
    g.append('path').datum(curveData).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)).attr('fill','url(#animatedAreaGrad)').attr('stroke',colors[0]).attr('stroke-width',1.6)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Animated Gradient')\r
