...
export const meta = {\r
  id: 'area-with-online-learning-regret',\r
  title: 'Area With Online Learning Regret',\r
  desc: 'Area With Online Learning Regret — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithOnlineLearningRegret',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-online-learning-regret"],\r
}\r
\r
export default function AreaWithOnlineLearningRegret({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Online learning regret\r
    let cumRegret=0\r
    const regretData=curveData.map((d,i)=>({x:d.x, regret:cumRegret+=Math.abs(d.y-42)*0.1}))\r
    ...
    // Cumulative regret line\r
    g.append('path').datum(regretData).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.regret*5)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#ef4444').attr('stroke-width',1.4).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Online Learning Regret')\r
