...
export const meta = {\r
  id: 'area-with-cat-boost-trend',\r
  title: 'Area With Cat Boost Trend',\r
  desc: 'Area With Cat Boost Trend — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithCatBoostTrend',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-cat-boost-trend"],\r
}\r
\r
export default function AreaWithCatBoostTrend({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // CatBoost-style gradient boosting\r
    const trees=100\r
    const lr=0.05\r
    let predictions=curveData.map(d=>d.y)\r
    for(let t=0;t<20;t++){\r
      const residuals=curveData.map((d,i)=>({x:curveData[i].x, y:curveData[i].y-predictions[i]}))\r
      const tree=residuals.map(r=>({x:r.x, y:r.y*0.1}))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with CatBoost Trend')\r
