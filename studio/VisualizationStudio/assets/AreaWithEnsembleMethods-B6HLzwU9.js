...
export const meta = {\r
  id: 'area-with-ensemble-methods',\r
  title: 'Area With Ensemble Methods',\r
  desc: 'Area With Ensemble Methods — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithEnsembleMethods',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-ensemble-methods"],\r
}\r
\r
export default function AreaWithEnsembleMethods({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Multiple ensemble methods\r
    const methods=['Bagging','Boosting','Stacking','Blending','Voting']\r
    const methodsData=methods.map(m=>curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*4})))\r
    const ensemble=curveData.map((_,i)=>({x:curveData[i].x, y:d3.mean(methodsData.map(m=>m[i].y))}))\r
    // Individual methods\r
    methodsData.forEach((method,i)=>{\r
      g.append('path').datum(method).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Ensemble Methods')\r
