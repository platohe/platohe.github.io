...
export const meta = {\r
  id: 'area-with-policy-gradient',\r
  title: 'Area With Policy Gradient',\r
  desc: 'Area With Policy Gradient — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithPolicyGradient',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-policy-gradient"],\r
}\r
\r
export default function AreaWithPolicyGradient({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    curveData.forEach(d=>{\r
      const action=d.y-42\r
      const reward=-(d.y-42)*(d.y-42)/100\r
      logProb+=action*reward\r
      trajectory.push({x:d.x, y:42+logProb*10})\r
    })\r
...
g.append('path').datum(trajectory).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
.attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1.4).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Policy Gradient')\r
