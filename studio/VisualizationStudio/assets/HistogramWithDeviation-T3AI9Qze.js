...
export const meta = {\r
  id: 'histogram-with-deviation',\r
  title: 'Histogram With Deviation',\r
  desc: 'Histogram With Deviation — a analysis chart visualization',\r
  category: 'Analysis',\r
  component: 'HistogramWithDeviation',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["analysis","histogram-with-deviation"],\r
}\r
\r
export default function HistogramWithDeviation({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    g.append('line').attr('x1',x(avg)).attr('x2',x(avg)).attr('y1',0).attr('y2',height).attr('stroke',dev>=0?'#22c55e':'#ef4444').attr('stroke-width',1.2).attr('stroke-dasharray','3,2')\r
    g.append('text').attr('x',x(avg)+4).attr('y',10).attr('fill',dev>=0?'#22c55e':'#ef4444').attr('font-size','7px').text(\`\${dev>=0?'+':''}\${dev.toFixed(1)}\`)\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Histogram with Deviation')\r
  },[customData])\r
