...
export const meta = {\r
  id: 'area-with-cusumchange-point',\r
  title: 'Area With C U S U M Change Point',\r
  desc: 'Area With C U S U M Change Point — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithCUSUMChangePoint',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-c-u-s-u-m-change-point"],\r
}\r
\r
export default function AreaWithCUMSUMChangePoint({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // CUSUM change point\r
    let cusum=0, k=1, h=5\r
    const cusumData=curveData.map((d,i)=>{\r
      if(i===0) return {x:d.x, cusum:0}\r
      const diff=d.y-42\r
      cusum=Math.max(0, cusum + diff - 1)\r
      return {x:d.x, cusum}\r
    })\r
    ...
    g.append('path').datum(cusumData).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.cusum*2)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#ef4444').attr('stroke-width',1.2).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with CUSUM Change Point')\r
