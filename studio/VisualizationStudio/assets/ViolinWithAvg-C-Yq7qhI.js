...
export const meta = {\r
  id: 'violin-with-avg',\r
  title: 'Violin With Avg',\r
  desc: 'Violin With Avg — a analysis chart visualization',\r
  category: 'Analysis',\r
  component: 'ViolinWithAvg',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["analysis","violin-with-avg"],\r
}\r
\r
export default function ViolinWithAvg({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    groups.forEach((gr,i)=>{\r
      ...
      g.append('line').attr('x1',cx-8).attr('x2',cx+8).attr('y1',y(avg)).attr('y2',y(avg)).attr('stroke','#0f172a').attr('stroke-width',1.2).attr('stroke-dasharray','2,2')\r
      g.append('circle').attr('cx',cx).attr('cy',y(avg)).attr('r',2).attr('fill','#0f172a')\r
      g.append('circle').attr('cx',cx).attr('cy',y(vals[Math.floor(vals.length/2)])).attr('r',1.6).attr('fill',colors[i]).attr('opacity',0.62)\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Violin with Avg')\r
  },[customData])\r
