...
export const meta = {\r
  id: 'area-with-rolling-kurtosis',\r
  title: 'Area With Rolling Kurtosis',\r
  desc: 'Area With Rolling Kurtosis — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithRollingKurtosis',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-rolling-kurtosis"],\r
}\r
\r
export default function AreaWithRollingSkewness({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Rolling skewness\r
    const window=20\r
    const skew=curveData.map((_,i)=>{\r
      if(i<10||i>=curveData.length-10) return {x:curveData[i].x, skew:0}\r
      const slice=curveData.slice(i-10,i).map(d=>d.y)\r
      const mean=d3.mean(slice)||42, std=d3.deviation(slice)||1\r
      return {x:curveData[i].x, skew:d3.mean(slice.map(v=>Math.pow((v-mean)/std,3)))||0}\r
    })\r
    ...
    g.append('path').datum(skew).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.skew)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1.4).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Rolling Skewness')\r
