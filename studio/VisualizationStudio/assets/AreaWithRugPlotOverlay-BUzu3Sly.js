...
export const meta = {\r
  id: 'area-with-rug-plot-overlay',\r
  title: 'Area With Rug Plot Overlay',\r
  desc: 'Area With Rug Plot Overlay — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithRugPlotOverlay',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-rug-plot-overlay"],\r
}\r
\r
export default function AreaWithRugPlotOverlay({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    curveData.forEach(d=>{\r
      g.append('line').attr('x1',x(d.x)).attr('x2',x(d.x)).attr('y1',height).attr('y2',height-6).attr('stroke',colors[0]).attr('stroke-width',0.8).attr('opacity',0.5)\r
    })\r
    // Rug plot at top (for values above mean)\r
    const mean=42\r
    curveData.filter(d=>d.y>42).forEach(d=>{\r
      g.append('line').attr('x1',x(d.x)).attr('x2',x(d.x)).attr('y1',0).attr('y2',6).attr('stroke','#22c55e').attr('stroke-width',0.8).attr('opacity',0.5)\r
    })\r
    g.append('path').datum(curveData).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis))\r
      .attr('fill',colors[0]).attr('fill-opacity',0.18).attr('stroke',colors[0]).attr('stroke-width',1.6)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Rug Plot Overlay')\r
