...
export const meta = {\r
  id: 'bean-with-outliers',\r
  title: 'Bean With Outliers',\r
  desc: 'Bean With Outliers — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'BeanWithOutliers',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","bean-with-outliers"],\r
}\r
\r
export default function BeanWithOutliers({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    raw.forEach((vals,i)=>{\r
      ...
      vals.filter(v=>v<lo||v>hi).forEach(v=> g.append('circle').attr('cx',cx+(Math.random()-0.5)*6).attr('cy',y(v)).attr('r',2.2).attr('fill','#ef4444').attr('stroke','var(--bg)'))\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Bean with Outliers')\r
  },[customData])\r
