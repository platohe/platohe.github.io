...
export const meta = {\r
  id: 'bean-with-avg',\r
  title: 'Bean With Avg',\r
  desc: 'Bean With Avg — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'BeanWithAvg',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","bean-with-avg"],\r
}\r
\r
export default function BeanWithAvg({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    raw.forEach((vals,i)=>{\r
      ...
      g.append('line').attr('x1',cx-10).attr('x2',cx+10).attr('y1',y(avg)).attr('y2',y(avg)).attr('stroke','#0f172a').attr('stroke-width',1.2).attr('stroke-dasharray','2,2')\r
      g.append('circle').attr('cx',cx).attr('cy',y(avg)).attr('r',2).attr('fill','#0f172a')\r
      const m=d3.median(sorted)||50\r
      g.append('circle').attr('cx',cx).attr('cy',y(m)).attr('r',1.6).attr('fill',colors[i]).attr('opacity',0.62)\r
    })\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Bean with Avg')\r
  },[customData])\r
