...
export const meta = {\r
  id: 'qqwith-avg',\r
  title: 'Q Q With Avg',\r
  desc: 'Q Q With Avg — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'QQWithAvg',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","q-q-with-avg"],\r
}\r
\r
export default function QQWithAvg({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    g.append('circle').attr('cx',x(avgExp)).attr('cy',y(avgObs)).attr('r',3).attr('fill','#0f172a').attr('stroke','var(--bg)')\r
    g.append('text').attr('x',x(avgExp)+4).attr('y',y(avgObs)-6).attr('fill','#0f172a').attr('font-size','7px').text('avg')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Q-Q with Avg')\r
  },[customData])\r
