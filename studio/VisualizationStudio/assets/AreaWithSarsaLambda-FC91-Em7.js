...
export const meta = {\r
  id: 'area-with-sarsa-lambda',\r
  title: 'Area With Sarsa Lambda',\r
  desc: 'Area With Sarsa Lambda — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithSarsaLambda',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-sarsa-lambda"],\r
}\r
\r
export default function AreaWithSarsaLambda({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // SARSA(lambda)\r
    const lambda=0.8, gamma=0.9\r
    let eligibility=0, value=42\r
    const trace=[]\r
    curveData.forEach((d,i)=>{\r
      const td=d.y-value\r
      value+=0.1*td\r
      eligibility=0.9*eligibility+0.9\r
      trace.push({x:d.x, y:value})\r
    })\r
    ...
    g.append('path').datum(trace).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1.4).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with SARSA(lambda)')\r
