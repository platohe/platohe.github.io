...
export const meta = {\r
  id: 'area-with-trailing-stop',\r
  title: 'Area With Trailing Stop',\r
  desc: 'Area With Trailing Stop — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithTrailingStop',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-trailing-stop"],\r
}\r
\r
export default function AreaWithTrailingStop({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Trailing stop (ATR-based)\r
    const window=5\r
    const stops=curveData.map((d,i)=>{\r
      const slice=curveData.slice(Math.max(0,i-14),i+1)\r
      const vals=slice.map(d=>d.y)\r
      const atr=d3.mean(slice.map((d,j)=>j>0?Math.abs(slice[j].y-slice[j-1].y):0))||0\r
      const trend=d3.mean(slice.map(d=>d.y))||0\r
      return {x:d.x, stop: trend > d.y ? d.y + 2*atr : d.y - 2*atr}\r
    })\r
    ...
    g.append('path').datum(stops).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.stop)).curve(d3.curveBasis)).attr('fill','none').attr('stroke','#ef4444').attr('stroke-width',1.4).attr('stroke-dasharray','3,2')\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Trailing Stop')\r
