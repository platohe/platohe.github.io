...
export const meta = {\r
  id: 'area-with-pivot-points',\r
  title: 'Area With Pivot Points',\r
  desc: 'Area With Pivot Points — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithPivotPoints',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-pivot-points"],\r
}\r
\r
export default function AreaWithPivotPoints({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Pivot points\r
    const high=d3.max(curveData,d=>d.y)||0, low=d3.min(curveData,d=>d.y)||0, close=curveData[curveData.length-1].y||0\r
    const pp=(high+low+close)/3\r
    const r1=2*pp-low, s1=2*pp-high\r
    const r2=pp+high-low, s2=pp-high+low\r
    const r3=high+2*(pp-low), s3=pp-high+low\r
    const levels=[{label:'R3',v:r3},{label:'R2',v:r2},{label:'R1',v:r1},{label:'PP',v:pp},{label:'S1',v:s1},{label:'S2',v:s2},{label:'S3',v:s3}]\r
    levels.forEach(l=>{\r
      g.append('line').attr('x1',0).attr('x2',width).attr('y1',y(l.v)).attr('y2',y(l.v)).attr('stroke','#94a3b8').attr('stroke-dasharray','2,2').attr('opacity',0.5)\r
      g.append('text').attr('x',width-4).attr('y',y(l.v)-4).attr('text-anchor','end').attr('fill','#94a3b8').attr('font-size','6px').text(\`\${l.label} \${l.v.toFixed(1)}\`)\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Pivot Points')\r
