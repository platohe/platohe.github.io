...
export const meta = {\r
  id: 'area-with-diagonal-stripes',\r
  title: 'Area With Diagonal Stripes',\r
  desc: 'Area With Diagonal Stripes — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithDiagonalStripes',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-diagonal-stripes"],\r
}\r
\r
export default function AreaWithDiagonalStripes({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Diagonal stripe pattern\r
    const defs=svg.append('defs')\r
    const pattern=defs.append('pattern').attr('id','diagonalStripes').attr('patternUnits','userSpaceOnUse').attr('width',12).attr('height',12).attr('patternTransform','rotate(45)')\r
    pattern.append('rect').attr('width',12).attr('height',12).attr('fill',colors[0]).attr('fill-opacity',0.1)\r
    pattern.append('rect').attr('x',6).attr('y',0).attr('width',3).attr('height',12).attr('fill',colors[0]).attr('fill-opacity',0.3)\r
    const area=d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)\r
    g.append('path').datum(curveData).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)).attr('fill','url(#diagonalStripes)').attr('stroke',colors[0]).attr('stroke-width',1.6)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Diagonal Stripes')\r
