...
export const meta = {\r
  id: 'area-with-reinforcement-learning',\r
  title: 'Area With Reinforcement Learning',\r
  desc: 'Area With Reinforcement Learning — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithReinforcementLearning',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis","d3-hierarchy"],\r
  tags: ["areas","area-with-reinforcement-learning"],\r
}\r
\r
export default function AreaWithReinforcementLearning({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // RL agent path\r
    const episodes=10\r
    const episodesData=Array.from({length:10},()=>curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*4})))\r
    episodesData.forEach((ep,i)=>{\r
      g.append('path').datum(ep).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Reinforcement Learning')\r
