...
export const meta = {\r
  id: 'area-with-marginal-rugs',\r
  title: 'Area With Marginal Rugs',\r
  desc: 'Area With Marginal Rugs — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithMarginalRugs',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-marginal-rugs"],\r
}\r
\r
export default function AreaWithMarginalRugs({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Marginal rugs on both axes\r
    const rugLen=8\r
    curveData.forEach(d=>{\r
      g.append('line').attr('x1',x(d.x)).attr('x2',x(d.x)).attr('y1',height).attr('y2',height-6).attr('stroke',colors[0]).attr('stroke-width',0.6).attr('opacity',0.5)\r
      g.append('line').attr('x1',-6).attr('x2',0).attr('y1',y(d.y)).attr('y2',y(d.y)).attr('stroke',colors[0]).attr('stroke-width',0.6).attr('opacity',0.5)\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Marginal Rugs')\r
