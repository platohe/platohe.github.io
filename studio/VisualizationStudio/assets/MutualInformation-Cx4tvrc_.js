...
\r
export const meta = {\r
  id: 'mutual-information',\r
  title: 'Mutual Information',\r
  desc: 'Mutual Information — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'MutualInformation',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","mutual-information"],\r
}\r
\r
export default function MutualInformation({ data: customData }) {\r
  const ref = useRef(null)\r
\r
  const DEFAULT_DATA = [{"feature":"Age","mi":0.45},{"feature":"Income","mi":0.38},{"feature":"CreditScore","mi":0.32},{"feature":"Employment","mi":0.22},{"feature":"DebtRatio","mi":0.18},{"feature":"LoanAmount","mi":0.12},{"feature":"MaritalStatus","mi":0.08},{"feature":"Region","mi":0.04}]\r
\r
  useEffect(() => {\r
    const svg = d3.select(ref.current)\r
    svg.selectAll('*').remove()\r
\r
    const data = (Array.isArray(customData) && customData.length > 0 && customData[0]?.mi !== undefined) ? customData : DEFAULT_DATA\r
    const sorted = [...data].sort((a, b) => b.mi - a.mi)\r
    const maxVal = d3.max(sorted, d => d.mi) || 1\r
\r
...
\r
      svg.append('text').attr('transform', \`translate(\${M.left + barW + 4},\${M.top + y(d.feature) + barH / 2 + 4})\`)\r
        .attr('fill', 'var(--text-secondary)').attr('font-size', '10px').text(d.mi.toFixed(3))\r
    })\r
\r
    // Axes\r
    svg.append('g').attr('transform', \`translate(\${M.left},\${M.top})\`)\r
...
\r
    svg.append('text').attr('transform', \`translate(\${M.left + IW/2},\${M.top + IH + 28})\`)\r
      .attr('text-anchor', 'middle').attr('fill', 'var(--text-secondary)').attr('font-size', '11px').text('Mutual Information (nats)')\r
    svg.append('text').attr('transform', \`translate(\${M.left + IW/2},\${M.top - 10})\`)\r
      .attr('text-anchor', 'middle').attr('fill', 'var(--text-primary)').attr('font-size', '14px').attr('font-weight', 'bold').text('Mutual Information Feature Selection')\r
\r
  }, [customData])\r
\r
