...
export const meta = {\r
  id: 'area-with-neural-architecture-search',\r
  title: 'Area With Neural Architecture Search',\r
  desc: 'Area With Neural Architecture Search — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithNeuralArchitectureSearch',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis","d3-shape"],\r
  tags: ["areas","area-with-neural-architecture-search"],\r
}\r
\r
export default function AreaWithNeuralArchitectureSearch({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Neural architecture search paths\r
    const architectures=10\r
    const architecturesData=Array.from({length:10},()=>curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*4})))\r
    architecturesData.forEach((arch,i)=>{\r
      g.append('path').datum(arch).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Neural Architecture Search')\r
