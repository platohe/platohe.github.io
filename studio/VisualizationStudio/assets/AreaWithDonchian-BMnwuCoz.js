...
export const meta = {\r
  id: 'area-with-donchian',\r
  title: 'Area With Donchian',\r
  desc: 'Area With Donchian — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithDonchian',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-donchian"],\r
}\r
\r
export default function AreaWithDonchian({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    const channels=curveData.map((d,i)=>{\r
      ...
      const upper=d3.max(vals)||0\r
      const lower=d3.min(vals)||0\r
      const middle=(upper+lower)/2\r
      return {x:d.x, upper, lower, middle}\r
    })\r
    ...
    g.append('path').datum(channels).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.middle)).curve(d3.curveBasis)).attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1).attr('stroke-dasharray','3,2')\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Donchian Channels')\r
