...
export const meta = {\r
  id: 'area-with-bayesian-optimization',\r
  title: 'Area With Bayesian Optimization',\r
  desc: 'Area With Bayesian Optimization — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithBayesianOptimization',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-bayesian-optimization"],\r
}\r
\r
export default function AreaWithBayesianOptimization({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Bayesian optimization acquisition function\r
    const acq=curveData.map((d,i)=>({x:d.x, y:d.y + (Math.random()-0.5)*4}))\r
    // Expected improvement\r
    const ei=curveData.map((d,i)=>({x:d.x, y:42 + (Math.random()-0.5)*6}))\r
    g.append('path').datum(ei).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Bayesian Optimization')\r
