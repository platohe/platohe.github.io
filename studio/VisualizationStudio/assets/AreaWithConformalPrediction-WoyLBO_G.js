...
export const meta = {\r
  id: 'area-with-conformal-prediction',\r
  title: 'Area With Conformal Prediction',\r
  desc: 'Area With Conformal Prediction — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithConformalPrediction',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-conformal-prediction"],\r
}\r
\r
export default function AreaWithConformalPrediction({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    const alpha=0.1\r
    const scores=curveData.map(d=>Math.abs(d.y-42))\r
    const threshold=d3.quantile(scores.sort(d3.ascending), 0.9)||5\r
    const pred=42\r
    const lower=pred-threshold, upper=pred+threshold\r
    // Prediction interval band\r
    g.append('rect').attr('x',0).attr('y',y(upper)).attr('width',width).attr('height',y(lower)-y(upper))\r
    ...
    // Conformal band\r
    g.append('rect').attr('x',width-20).attr('y',y(upper)).attr('width',15).attr('height',y(lower)-y(upper))\r
      .attr('fill',colors[0]).attr('fill-opacity',0.2).attr('stroke',colors[0])\r
    g.append('text').attr('x',width-2).attr('y',y((lower+upper)/2)).attr('text-anchor','end').attr('fill','var(--text-secondary)').attr('font-size','6px').text('90%')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Conformal Prediction')\r
