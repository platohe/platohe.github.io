...
export const meta = {\r
  id: 'area-with-neural-ode',\r
  title: 'Area With Neural O D E',\r
  desc: 'Area With Neural O D E — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithNeuralODE',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-neural-o-d-e"],\r
}\r
\r
export default function AreaWithNeuralODE({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Neural ODE trend\r
    const steps=5\r
    const stepsData=Array.from({length:5},()=>curveData.map(d=>({x:d.x, y:d.y+(Math.random()-0.5)*2})))\r
    stepsData.forEach((step,i)=>{\r
      g.append('path').datum(step).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Neural ODE')\r
