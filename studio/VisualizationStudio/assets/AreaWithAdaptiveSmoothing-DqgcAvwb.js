...
export const meta = {\r
  id: 'area-with-adaptive-smoothing',\r
  title: 'Area With Adaptive Smoothing',\r
  desc: 'Area With Adaptive Smoothing — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithAdaptiveSmoothing',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-adaptive-smoothing"],\r
}\r
\r
export default function AreaWithAdaptiveSmoothing({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Adaptive smoothing - window size based on local variance\r
    const smoothed=curveData.map((d,i)=>{\r
      const localVar=d3.deviation(curveData.slice(Math.max(0,i-5),i+6).map(d=>d.y))||1\r
      const window=Math.max(3,Math.min(15,Math.round(5/localVar)))\r
      const start=Math.max(0,i-Math.floor(window/2))\r
      const end=Math.min(curveData.length-1,i+Math.floor(window/2))\r
      const slice=curveData.slice(start,end+1).map(d=>d.y)\r
      return {x:curveData[i].x, y:d3.mean(slice)||curveData[i].y}\r
    })\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Adaptive Smoothing')\r
