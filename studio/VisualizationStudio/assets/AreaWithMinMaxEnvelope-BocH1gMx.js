...
export const meta = {\r
  id: 'area-with-min-max-envelope',\r
  title: 'Area With Min Max Envelope',\r
  desc: 'Area With Min Max Envelope — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithMinMaxEnvelope',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-min-max-envelope"],\r
}\r
\r
export default function AreaWithMinMaxEnvelope({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    const envelopes=curveData.map((d,i)=>{\r
      ...
      const min=d3.min(slice,d=>d.y)||0\r
      const max=d3.max(slice,d=>d.y)||0\r
      return {x:d.x, min, max}\r
    })\r
    const area=d3.area().x(d=>x(d.x)).y0(d=>y(d.min)).y1(d=>y(d.max)).curve(d3.curveBasis)\r
    g.append('path').datum(envelopes).attr('d',area).attr('fill',colors[0]).attr('fill-opacity',0.14).attr('stroke',colors[0]).attr('stroke-width',1)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Min/Max Envelope')\r
