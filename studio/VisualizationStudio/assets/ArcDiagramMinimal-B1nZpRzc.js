...
// ArcDiagramMinimal: Thin monochrome metro-style lines.\r
export const meta = {\r
  id: 'arc-diagram-minimal',\r
  title: 'Arc Diagram Minimal',\r
  desc: 'Arc Diagram Minimal — a networks chart visualization',\r
  category: 'Networks',\r
  component: 'ArcDiagramMinimal',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-shape"],\r
  tags: ["networks","arc-diagram-minimal"],\r
}\r
\r
export default function ArcDiagramMinimal({ data: customData }) {\r
  const ref = useRef(null)\r
  useEffect(() => {\r
    const svg = d3.select(ref.current); svg.selectAll('*').remove()\r
    const DEFAULT_DATA = {"nodes":[{"id":"a1","name":"Central"},{"id":"a2","name":"Park"},{"id":"a3","name":"Riverside"},{"id":"a4","name":"Airport"},{"id":"a5","name":"Stadium"}],"links":[{"source":"a1","target":"a2","value":1},{"source":"a1","target":"a3","value":1},{"source":"a2","target":"a4","value":1},{"source":"a3","target":"a5","value":1},{"source":"a4","target":"a5","value":1}]}\r
...
\r
    ...
    data.links.forEach(lk => {\r
      ...
      g.append('path')\r
        ...
        .attr('fill','none').attr('stroke', 'var(--border)').attr('stroke-width', w).attr('stroke-opacity', 0.9)\r
    ...
    data.nodes.forEach((n,i)=>{\r
      ...
      g.append('text').attr('x', x).attr('y', y0+18).attr('text-anchor', flip?'end':'start').attr('dx', flip?-8:8)\r
        .attr('font-size','8px').attr('fill','var(--text-secondary)').text(n.name)\r
    })\r
