...
export const meta = {\r
  id: 'area-with-pattern-stops',\r
  title: 'Area With Pattern Stops',\r
  desc: 'Area With Pattern Stops — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithPatternStops',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-pattern-stops"],\r
}\r
\r
export default function AreaWithPatternStops({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    const pattern=defs.append('pattern').attr('id','areaPatternStops').attr('patternUnits','userSpaceOnUse').attr('width',20).attr('height',20)\r
    pattern.append('rect').attr('width',20).attr('height',20).attr('fill',colors[0]).attr('fill-opacity',0.3)\r
    for(let i=0;i<20;i+=4){\r
      pattern.append('circle').attr('cx',i+2).attr('cy',2).attr('r',1.5).attr('fill','#fff').attr('opacity',0.6)\r
      pattern.append('circle').attr('cx',i+2).attr('cy',18).attr('r',1.5).attr('fill','#fff').attr('opacity',0.6)\r
    }\r
    const area=d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)\r
    g.append('path').datum(curveData).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)).attr('fill','url(#areaPatternStops)').attr('stroke',colors[0]).attr('stroke-width',1.6)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Pattern Stops')\r
