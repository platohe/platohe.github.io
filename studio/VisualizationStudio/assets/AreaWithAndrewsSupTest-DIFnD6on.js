...
export const meta = {\r
  id: 'area-with-andrews-sup-test',\r
  title: 'Area With Andrews Sup Test',\r
  desc: 'Area With Andrews Sup Test — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithAndrewsSupTest',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-andrews-sup-test"],\r
}\r
\r
export default function AreaWithSupremumTest({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    g.append('text').attr('x',x(25)).attr('y',10).attr('text-anchor','middle').attr('fill','#ef4444').attr('font-size','6px').attr('font-weight',600).text('Sup Test')\r
    g.append('path').datum(curveData).attr('d',area).attr('fill',colors[0]).attr('fill-opacity',0.18).attr('stroke',colors[0]).attr('stroke-width',1.6)\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Supremum Test')\r
