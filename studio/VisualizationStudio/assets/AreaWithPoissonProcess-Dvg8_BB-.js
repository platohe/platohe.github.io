...
export const meta = {\r
  id: 'area-with-poisson-process',\r
  title: 'Area With Poisson Process',\r
  desc: 'Area With Poisson Process — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithPoissonProcess',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-poisson-process"],\r
}\r
\r
export default function AreaWithPoissonProcess({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Poisson process\r
    const lambda=1\r
    const poisson=Array.from({length:52},(_,i)=>({x:i, y: d3.randomPoisson(42)()}))\r
    // Poisson dots\r
    poisson.forEach(p=>{\r
      g.append('circle').attr('cx',x(p.x)).attr('cy',y(p.y)).attr('r',3).attr('fill',colors[0]).attr('opacity',0.6)\r
    })\r
    // Original area\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Poisson Process')\r
