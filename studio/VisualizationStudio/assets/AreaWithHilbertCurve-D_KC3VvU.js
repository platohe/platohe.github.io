...
export const meta = {\r
  id: 'area-with-hilbert-curve',\r
  title: 'Area With Hilbert Curve',\r
  desc: 'Area With Hilbert Curve — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithHilbertCurve',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-hilbert-curve"],\r
}\r
\r
export default function AreaWithHilbertCurve({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Hilbert curve area\r
    const n=6\r
    const size=width/(1<<n)\r
    // Local Hilbert index->xy (order n)\r
    const side=1<<n\r
    const hilbertPoint=(idx)=>{\r
      let t=idx, d=[0,0]\r
      for(let s=1;s<side;s*=2){\r
        const rx=1&(t/2), ry=1&(t^rx)\r
        if(ry===0){ if(rx===1){ d[0]=s-1-d[0]; d[1]=s-1-d[1] } const tmp=d[0]; d[0]=d[1]; d[1]=tmp }\r
        d[0]+=s*rx; d[1]+=s*ry; t=Math.floor(t/4)\r
      }\r
      return d\r
    }\r
    const points=Array.from({length:side*side},(_,i)=>hilbertPoint(i))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Hilbert Curve')\r
