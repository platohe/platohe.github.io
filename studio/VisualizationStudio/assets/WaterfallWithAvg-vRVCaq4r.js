...
export const meta = {\r
  id: 'waterfall-with-avg',\r
  title: 'Waterfall With Avg',\r
  desc: 'Waterfall With Avg — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'WaterfallWithAvg',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","waterfall-with-avg"],\r
}\r
\r
export default function WaterfallWithAvg({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    const yMax=Number.isFinite(yMaxRaw)? yMaxRaw*1.08:80\r
    const y=d3.scaleLinear().domain([0, yMax||80]).nice().range([height,0])\r
...
\r
    ...
    const avgRaw=d3.mean(raw.filter(d=>!d.isTotal),d=>d.v)\r
    const avg=Number.isFinite(avgRaw)?avgRaw:0\r
    g.append('line').attr('x1',0).attr('x2',width).attr('y1',safeY(avg+data[0].y0)).attr('y2',safeY(avg+data[0].y0)).attr('stroke','#f59e0b').attr('stroke-dasharray','3,2').attr('opacity',0.72)\r
    g.append('text').attr('x',width-2).attr('y',safeY(avg+data[0].y0)-4).attr('text-anchor','end').attr('fill','#f59e0b').attr('font-size','7px').text(\`avg \${avg.toFixed(1)}\`)\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Waterfall with Avg')\r
  },[customData])\r
