...
export const meta = {\r
  id: 'area-with-spectral-overlay',\r
  title: 'Area With Spectral Overlay',\r
  desc: 'Area With Spectral Overlay — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithSpectralOverlay',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-spectral-overlay"],\r
}\r
\r
export default function AreaWithSpectralOverlay({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // Spectral overlay using FFT-like bands\r
    const bands=5\r
    for(let b=0;b<bands;b++){\r
      const bandData=curveData.map(d=>({x:d.x, y:d.y + Math.sin(d.x*0.5+b*1.5)*3}))\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Spectral Overlay')\r
