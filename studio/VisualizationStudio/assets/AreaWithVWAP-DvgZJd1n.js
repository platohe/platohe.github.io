...
export const meta = {\r
  id: 'area-with-vwap',\r
  title: 'Area With V W A P',\r
  desc: 'Area With V W A P — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithVWAP',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-v-w-a-p"],\r
}\r
\r
export default function AreaWithVWAP({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // VWAP calculation\r
    let cumVol=0, cumPV=0\r
    const vwap=curveData.map(d=>{\r
      cumVol+=d.volume\r
      cumPV+=d.y*d.volume\r
      return {x:d.x, y:d.y, vwap: cumVol>0?cumPV/cumVol:0}\r
    })\r
    ...
    g.append('path').datum(vwap).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.vwap)).curve(d3.curveBasis))\r
      .attr('fill','none').attr('stroke','#f59e0b').attr('stroke-width',1.4).attr('stroke-dasharray','3,2')\r
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with VWAP')\r
