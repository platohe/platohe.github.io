...
export const meta = {\r
  id: 'area-with-gradient-stops',\r
  title: 'Area With Gradient Stops',\r
  desc: 'Area With Gradient Stops — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithGradientStops',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-gradient-stops"],\r
}\r
\r
export default function AreaWithGradientStops({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    const grad=defs.append('linearGradient').attr('id','areaGradStops').attr('x1','0%').attr('y1','0%').attr('x2','0%').attr('y2','100%')\r
    grad.append('stop').attr('offset','0%').attr('stop-color',colors[0]).attr('stop-opacity',0.8)\r
    grad.append('stop').attr('offset','25%').attr('stop-color',colors[1]).attr('stop-opacity',0.6)\r
    grad.append('stop').attr('offset','50%').attr('stop-color',colors[2]).attr('stop-opacity',0.4)\r
    grad.append('stop').attr('offset','75%').attr('stop-color',colors[3]).attr('stop-opacity',0.2)\r
    grad.append('stop').attr('offset','100%').attr('stop-color',colors[4]).attr('stop-opacity',0.1)\r
    g.append('path').datum(curveData).attr('d',d3.area().x(d=>x(d.x)).y0(y(0)).y1(d=>y(d.y)).curve(d3.curveBasis)).attr('fill','url(#areaGradStops)').attr('stroke',colors[0]).attr('stroke-width',1.6)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with Gradient Stops')\r
