...
export const meta = {\r
  id: 'notched-with-outliers',\r
  title: 'Notched With Outliers',\r
  desc: 'Notched With Outliers — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'NotchedWithOutliers',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","notched-with-outliers"],\r
}\r
\r
export default function NotchedWithOutliers({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    raw.forEach((vals,i)=>{\r
      ...
      const n=vals.length, notch=1.58*(q3i-q1i)/Math.sqrt(n)\r
      const cx=(x(groups[i])??0)+x.bandwidth()/2, bw=x.bandwidth()*0.62\r
      const yQ1=y(q1i), yQ3=y(q3i), yM=y(m), yNM=y(m-notch), yPM=y(m+notch)\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Notched with Outliers')\r
  },[customData])\r
