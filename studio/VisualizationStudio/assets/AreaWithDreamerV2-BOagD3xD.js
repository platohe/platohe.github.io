...
export const meta = {\r
  id: 'area-with-dreamer-v2',\r
  title: 'Area With Dreamer V2',\r
  desc: 'Area With Dreamer V2 — a areas chart visualization',\r
  category: 'Areas',\r
  component: 'AreaWithDreamerV2',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale","d3-axis"],\r
  tags: ["areas","area-with-dreamer-v2"],\r
}\r
\r
export default function AreaWithDreamerV2({ data: customData, onCurveData }) {\r
...
\r
  ...
  useEffect(()=>{\r
    ...
    // DreamerV2 world model\r
    const horizon=10\r
    let imagined=curveData.map((d,i)=>({x:d.x, y:d.y + (Math.random()-0.5)*4}))\r
    for(let h=0;h<10;h++){\r
      g.append('path').datum(imagined).attr('d',d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveBasis))\r
        .attr('fill','none').attr('stroke',colors[0]).attr('stroke-width',0.8).attr('opacity',0.2)\r
      imagined=imagined.map(pt=>({x:pt.x, y: pt.y + (Math.random()-0.5)*2}))\r
    }\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Area with DreamerV2')\r
