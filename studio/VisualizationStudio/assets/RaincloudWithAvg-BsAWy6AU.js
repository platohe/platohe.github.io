...
export const meta = {\r
  id: 'raincloud-with-avg',\r
  title: 'Raincloud With Avg',\r
  desc: 'Raincloud With Avg — a bars chart visualization',\r
  category: 'Bars',\r
  component: 'RaincloudWithAvg',\r
  complexity: 'beginner',\r
  interactivity: ["none"],\r
  d3Api: ["d3-scale"],\r
  tags: ["bars","raincloud-with-avg"],\r
}\r
\r
export default function RaincloudWithAvg({ data: customData }) {\r
  ...
  useEffect(()=>{\r
    ...
    groups.forEach((gr,i)=>{\r
      ...
      g.append('line').attr('x1',x(avg)).attr('x2',x(avg)).attr('y1',yy- h(kde.find(k=>Math.abs(k.v-avg)<1)?.d||0)).attr('y2',yy+6).attr('stroke','#0f172a').attr('stroke-width',1).attr('stroke-dasharray','2,2')\r
    ...
    svg.append('text').attr('x',200).attr('y',14).attr('text-anchor','middle').attr('fill','var(--text)').attr('font-size','11px').attr('font-weight',600).text('Raincloud with Avg')\r
  },[customData])\r
